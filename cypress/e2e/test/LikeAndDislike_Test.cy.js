describe('Like and Dislike Feature Test', () => {
  it('should login, navigate to a specific post, like and reset like and comment, then logout', () => {
      ...
      cy.get(':nth-child(1) > :nth-child(1) > .ant-btn > .flex').click();
      cy.wait(3000);
      
      // Click on the like button again to reset the like
      cy.get(':nth-child(2) > :nth-child(1) > .ant-btn > .flex').click();
      cy.wait(3000);
      ...
      cy.get('.ant-slider-step > [style="left: 75%; transform: translateX(-50%);"]').click();
      cy.wait(1000);
  
      cy.get('.justify-center > .ant-btn > :nth-child(1)').click();
      cy.wait(3000);
