...
async function handler(
  ...
  if (isNaN(authCode))
    return res
      .status(400)
      .json({ message: 'Invalid auth code in request body.' });
  ...
  if (!user.two_factor_auth?.base32_secret)
    return res.status(400).json({ message: messages.TWO_FACTOR_AUTH_NOT_INIT });
  ...
  const isValidToken =
    totp.validate({ token: String(authCode).replaceAll(/\s/g, '') }) !== null;
  if (!isValidToken)
    return res
      .status(400)
  ...
  await firestore_db
    ...
    .catch((error) => {
      ...
      return res
        .status(500)
        .json({ message: 'Error verifying two factor auth code.' });
    });
