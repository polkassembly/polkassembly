...
async function handler(req: NextApiRequest, res: NextApiResponse<ChangeResponseType | MessageType>) {
	...
	const { address, signature , wallet, loginAddress } = req.body;
	...
	const updatedJWT = await authServiceInstance.AddressLinkConfirm(token, substrateAddress, signature, loginAddress, wallet);
