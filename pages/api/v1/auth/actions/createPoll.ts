...
const handler: NextApiHandler<ICreatePollResponse | MessageType> = async (req, res) => {
	...
	if (
		...
		(strPollType === 'option' && ((!endAt && endAt !== 0) || !optionsString || !question))
	)
	...
	if (!postDoc.exists) return res.status(400).json({ message: 'Post does not exist' });
	...
	const pollRef = postDocRef.collection(getPollCollectionName(strPollType)).doc();
	if (!poll) {
