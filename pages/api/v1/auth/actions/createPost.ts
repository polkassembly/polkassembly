...
async function handler(
	...
	const { content, proposalType, title, topicId, userId, gov_type, tags } =
		req.body;
	if (!content || !title || !topicId || !userId || !proposalType)
		return res
			.status(400)
	...
	const lastPostQuerySnapshot = await postsByTypeRef(
		...
		.limit(1)
		.get();
	const postsCount = lastPostQuerySnapshot.empty
		? 0
		: lastPostQuerySnapshot.docs[0].data().id || 0;
