...
const handler: NextApiHandler<MessageType> = async (req, res) => {
	...
	if (!pollId || isNaN(postId) || !userId) return res.status(400).json({ message: 'Missing parameters in request body' });
	...
	if (!updated[votes_field_name] || !Array.isArray(updated[votes_field_name])) {
		return res.status(500).json({ message: `The pollType "${pollType}" is invalid` });
	}
	const initialVotes = updated[votes_field_name].length;
	updated[votes_field_name] = updated[votes_field_name].filter((vote: any) => vote.user_id !== user.id);
	if (initialVotes === updated[votes_field_name].length) return res.status(400).json({ message: 'No vote found for the user' });
	...
	pollRef
		...
		.catch((error) => {
			...
			return res.status(500).json({ message: 'Error deleting poll vote' });
		});
};
