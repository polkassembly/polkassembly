...
const handler: NextApiHandler<IEditPostResponse | MessageType> = async (
  ...
  if (proposalType === ProposalType.ANNOUNCEMENT) {
    if (!postId || !title || !content || !proposalType)
      return res
        .status(400)
        .json({ message: 'Missing parameters in request body' });
  } else {
    if (isNaN(postId) || !title || !content || !proposalType)
      return res
        .status(400)
        .json({ message: 'Missing parameters in request body' });
  }
  ...
  if (postDoc.exists && !isNaN(post?.user_id)) {
    if (
      ![ProposalType.DISCUSSIONS, ProposalType.GRANTS].includes(proposalType)
    ) {
      ...
      const post =
        postRes.data?.proposals?.[0] || postRes.data?.announcements?.[0];
      if (!post)
        return res.status(500).json({ message: 'Something went wrong.' });
      if (!post?.proposer && !post?.preimage?.proposer)
        return res.status(500).json({ message: 'Something went wrong.' });
      ...
      isAuthor = Boolean(
        userAddresses.find((address) => address.address === substrateAddress),
      );
      if (
        network === 'moonbeam' &&
        ...
        if (
          userAddresses.find(
            (address) =>
              address.address === '0xbb1e1722513a8fa80f7593617bb0113b1258b7f1',
          )
        ) {
          isAuthor = true;
        }
      ...
      if (
        ...
        if (
          userAddresses.find(
            (address) =>
              address.address === '0x16095c509f728721ad19a51704fc39116157be3a',
          )
        ) {
          isAuthor = true;
        }
  ...
  const history =
    post?.history && Array.isArray(post?.history)
  ...
  if (timeline && Array.isArray(timeline) && timeline.length > 0) {
    ...
    timeline.forEach((obj) => {
      ...
      if (
        ...
        Number(obj.index) === Number(postId)
      ) {
        ...
        batch.set(
          postDocRef,
          {
            ...
            created_at,
            id:
              proposalType === ProposalType.TIPS ? obj.hash : Number(obj.index),
            last_edited_at: last_comment_at,
            post_link: post_link,
            proposer_address: proposer_address,
            tags: tags || [],
