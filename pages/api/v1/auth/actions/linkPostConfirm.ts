...
export const updatePostLinkInGroup: TUpdatePostLinkInGroup = async (params) => {
  ...
  const isAuthor =
    ...
    (isOffChainPost ? true : currPostData && user.id === currPostData.user_id);
  if (!isAuthor) {
    throw apiErrorWithStatusCode(
      `You can not ${
  ...
  if (proposals || Array.isArray(proposals)) {
    (proposals as any[]).forEach((proposal) => {
      if (proposal && proposal.type) {
        ...
        const id =
          proposal.type === 'Tip' ? proposal.hash : Number(proposal.index);
...
export const getPostsRefAndData: TGetPostsRefAndData = async (params) => {
  ...
  const refs = (
    postsRefWithData && Array.isArray(postsRefWithData) ? postsRefWithData : []
  ).map(({ ref }) => ref);
  if (refs.length > 0) {
...
const handler: NextApiHandler<ILinkPostConfirmResponse | MessageType> = async (
  ...
  try {
    ...
    return res
      .status(
        error.name && !isNaN(Number(error.name)) ? Number(error.name) : 500,
      )
