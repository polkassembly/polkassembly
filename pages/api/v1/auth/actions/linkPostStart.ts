...
const handler: NextApiHandler<ILinkPostStartResponse | MessageType> = async (
  ...
  if (isOffChainPost) {
    ...
    linkPostRes.created_at =
      postData?.created_at && postData?.created_at?.toDate
    ...
    if (!isDataExist(subsquidData)) {
      return res
        .status(400)
        .json({ message: `The Post with index "${postId}" is not found.` });
    }
    ...
    const proposerAddress =
      post.proposer || post.preimage?.proposer || post?.curator;
    ...
    const isAuthor =
      userAddresses.some((address) => address.address === substrateAddress) ||
    ...
    if (!linkPostRes.description) {
      linkPostRes.description =
        post.description || preimage?.proposedCall?.description;
    }
