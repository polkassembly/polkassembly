...
async function handler(
  ...
  const postRef = networkDocRef(network)
  ...
  if (!postSubs.includes(user.id))
    return res
      .status(400)
      .json({ message: messages.SUBSCRIPTION_REMOVE_SUCCESSFUL });
  ...
  await postRef
    ...
    .catch((error) => {
      ...
      return res
        .status(400)
        .json({ message: 'Error while removing user from post subscribers.' });
    });
