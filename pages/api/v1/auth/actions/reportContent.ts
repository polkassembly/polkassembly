...
async function handler(
  ...
  if (
    (type === 'post' && !post_id) ||
    (type === 'comment' && !(post_id && comment_id)) ||
    (type === 'reply' && !(post_id && comment_id && reply_id))
  ) {
    return res
      .status(400)
      .json({ message: 'Missing parameters in request body' });
  ...
  if (
    !isOffChainProposalTypeValid(proposalType) &&
    !isProposalTypeValid(proposalType)
  ) {
    return res
      .status(400)
      .json({ message: `Proposal type ${proposalType} is not valid.` });
  }
  ...
  if (!['post', 'comment', 'reply'].includes(type))
    return res.status(400).json({ message: messages.REPORT_TYPE_INVALID });
  if (!reason)
    return res.status(400).json({ message: messages.REPORT_REASON_REQUIRED });
  if (comments.length > 300)
    return res
      .status(400)
      .json({ message: messages.REPORT_COMMENTS_LENGTH_EXCEEDED });
  ...
  await networkDocRef(network)
    .collection('reports')
    .doc(`${user.id}_${contentId}_${strPostType}`)
    .set(data, { merge: true })
    .then(async () => {
    ...
    .catch((error) => {
      ...
      return res
        .status(500)
        .json({ message: 'Error while reporting content.' });
    });
}
