...
const handler: NextApiHandler<CreatePostResponseType> = async (req, res) => {
	...
	await postDocRef
		.set(newPost)
		.then(() => {
			return res.status(200).json({ message: 'Bounty Proposal successfully created, it will appear on polkassembly as soon as it is synced on chain.', post_id: postId });
		})
		.catch((error) => {
