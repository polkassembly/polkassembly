...
async function handler(req: NextApiRequest, res: NextApiResponse<Subscription | MessageType>) {
	...
	if (isNaN(numPostId)) {
		return res.status(400).json({ message: `The postId "${post_id}" is invalid.` });
	}
	...
	const userPreferenceDoc = await networkDocRef(network).collection('user_preferences').doc(strProposalType).get();
	if (userPreferenceDoc.exists) {
		...
		if (data) {
			...
			subscribed = post_subscriptions?.[strProposalType as keyof typeof post_subscriptions]?.some((id) => String(id) === String(post_id)) || false;
		} else {
			subscribed = false;
		}
	}
