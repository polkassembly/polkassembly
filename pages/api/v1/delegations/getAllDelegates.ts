...
interface IDelegateProfileDetails {
	[key: string]: { address: string; bio: string; image: string; dataSource: string[] };
}

const getDelegatesDataSourceAndDetails = (data: { address: string; bio: string; image: string; dataSource: string[] }[]) => {
	const res: IDelegateProfileDetails = {};

	data.map((item) => {
		if (res[item.address] == undefined) {
			res[item.address] = item;
		} else {
			const { address, bio, dataSource, image } = res[item?.address];
			if (dataSource.includes('polkassembly') && bio.length) {
				res[item?.address] = { address: address, bio: bio, dataSource: [...dataSource, ...(item?.dataSource || [])], image: item.image || image };
			} else {
				res[item?.address] = { address: address, bio: item?.bio || bio, dataSource: [...dataSource, ...(item?.dataSource || [])], image: item.image || image };
			}
		}
	});
	return res;
...
const getAllCombineDelegatesData = (
	delegatesDetails: IDelegateProfileDetails,
	delegatesStats: {
...
export const getDelegatesData = async (network: string, address?: string) => {
	...
	const totalDelegatorsObj: { [key: string]: { receivedDelegationsCount: { [key: string]: number }; delegatedBalance: BN; address: string; votedProposalCount?: number } } = {};

	data['data']?.votingDelegations.map((delegation: { to: string; balance: string; from: string; lockPeriod: number }) => {
		if (totalDelegatorsObj[delegation?.to] === undefined) {
			...
			totalDelegatorsObj[delegation?.to] = { address: delegation?.to, delegatedBalance: balance, receivedDelegationsCount: { [delegation.from]: 1 } };
		} else {
			...
			totalDelegatorsObj[delegation?.to] = {
				...
				receivedDelegationsCount: { ...totalDelegatorsObj[delegation?.to]?.receivedDelegationsCount, [delegation.from]: 1 }
			};
		}
	});
	...
	const votesCounts: { [key: string]: number } = {};
	...
	const delegatesDetails = getDelegatesDataSourceAndDetails([
		...novaWalletDelegates.map((item: any) => {
			return { address: getEncodedAddress(item.address, network), bio: item?.longDescription, dataSource: ['nova'], image: item.image || '' };
		}),
		...W3fDelegates.map((item: any) => {
			return { address: getEncodedAddress(item.address, network), bio: item?.longDescription || '', dataSource: ['w3f'], image: '' };
		}),
		...parityDelegates.map((item: any) => {
			return { address: getEncodedAddress(item.address, network), bio: item?.manifesto || '', dataSource: ['parity'], image: item?.image || '' };
		}),
		...paDelegates.map((item: any) => {
			return { address: getEncodedAddress(item.address, network), bio: item?.bio || '', dataSource: ['polkassembly'], image: item?.image || '' };
		})
	]);
