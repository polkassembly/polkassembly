  ...
  for (const [index, trackDelegationData] of subsquidResults.entries()) {
    ...
    const trackDelegation: ITrackDelegation = {
      active_proposals_count:
        trackDelegationData.value.data.proposalsConnection?.totalCount || 0,
    ...
    if (
      ...
      trackDelegation.recieved_delegation_count = votingDelegationsArr.filter(
...
async function handler(
  ...
  if (!address)
    return res.status(400).json({ error: 'Missing address in request query.' });

  const trackNum = Number(track);
  if (track && isNaN(trackNum))
    return res.status(400).json({ error: 'Invalid track in request query.' });
