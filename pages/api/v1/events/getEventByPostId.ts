...
const handler: NextApiHandler<NetworkEvent | MessageType> = async (req, res) => {
	...
	const eventQuerySnapshot = await firestore.collection('networks').doc(network).collection('events').where('post_id', '==', Number(post_id)).get();
	if (eventQuerySnapshot.empty) return res.status(404).json({ message: 'No events found for this post' });
