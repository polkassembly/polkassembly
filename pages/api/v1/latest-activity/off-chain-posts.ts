...
export async function getLatestActivityOffChainPosts(params: IGetLatestActivityOffChainPostsParams): Promise<IApiResponse<ILatestActivityPostsListingResponse>> {
	try {
		...
		const postsSnapshotArr = await postsColRef.where('isSpamDetected', '==', false).where('isDeleted', '==', false).orderBy('created_at', 'desc').limit(numListingLimit).get();
		const count = (await postsColRef.where('isDeleted', '==', false).where('isSpamDetected', '!=', true).count().get()).data().count;
