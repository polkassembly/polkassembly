...
import { isCustomOpenGovStatusValid, isProposalTypeValid, isValidNetwork } from '~src/api-utils';
...
export async function getOnChainPostsCount(params: IGetOnChainPostsCountParams): Promise<IApiResponse<IPostsListingResponse>> {
	try {
		const { network, page, proposalType, trackStatus } = params;
...
const handler: NextApiHandler<IPostsListingResponse | { error: string }> = async (req, res) => {
	const { page = 1, trackStatus, proposalType } = req.query;
