...
import { isCustomOpenGovStatusValid, isProposalTypeValid, isSortByValid, isValidNetwork } from '~src/api-utils';
...
export const fetchSubsquare = async (network: string, limit: number, page: number) => {
	try {
		const res = await fetch(`https://${network}.subsquare.io/api/gov2/referendums?page=${page}&page_size=${limit}`);
...
export async function getOnChainPosts(params: IGetOnChainPostsParams): Promise<IApiResponse<IPostsListingResponse>> {
	try {
		const { listingLimit, network, page, proposalType, sortBy, trackStatus, postIds, filterBy } = params;
		...
		if (filterBy && Array.isArray(filterBy) && filterBy.length > 0) {
			...
			try {
				...
				const data = await fetchSubsquare(network, Number(listingLimit), Number(page));
...
const handler: NextApiHandler<IPostsListingResponse | { error: string }> = async (req, res) => {
	const { page = 1, trackStatus, proposalType, sortBy = sortValues.NEWEST, listingLimit = LISTING_LIMIT, filterBy } = req.query;
