...
export async function getOnChainPosts(
  ...
  try {
    ...
    if (filterBy && Array.isArray(filterBy) && filterBy.length > 0) {
        ...
        async (subsquidPost): Promise<IPostListing> => {
          ...
          if (postDoc && postDoc.exists) {
            ...
            if (data) {
              ...
              const proposer_address = getProposerAddressFromFirestorePostData(
                data,
                network,
              );
      ...
      const orderBy =
        strSortBy === 'newest' ? 'createdAtBlock_DESC' : 'createdAtBlock_ASC';
      const postsVariables: any = {
        limit: numListingLimit,
        offset: numListingLimit * (numPage - 1),
        orderBy: orderBy,
      ...
      if (
        ...
        if (proposalType === ProposalType.ANNOUNCEMENT) {
          ...
          postsPromise = subsquidPosts?.map(async (subsquidPost) => {
            const { createdAt, description, hash, proposer, type, end, index } =
              subsquidPost;
          ...
          if (index !== undefined && index !== null) {
            if (
              ...
              posts[index] = {
                ...posts[index],
                spam_users_count:
                  Number(posts[index].spam_users_count || 0) + 1,
              };
            }
          }
        }
