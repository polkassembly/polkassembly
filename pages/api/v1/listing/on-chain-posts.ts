...
export function getProposerAddressFromFirestorePostData(
	data: any,
	network: string,
) {
	...
	return (
		(proposer_address && getEncodedAddress(proposer_address, network)) ||
		proposer_address
	);
}

export async function getOnChainPosts(
	params: IGetOnChainPostsParams,
	...
	try {
		...
		if (filterBy && Array.isArray(filterBy) && filterBy.length > 0) {
			...
			const postsSnapshotArr = await offChainCollRef
				...
				.offset(
					(Number(page) - 1) * Number(listingLimit || LISTING_LIMIT),
				)
				.get();
			...
			const postsPromise = postsSnapshotArr.docs.map(async (doc) => {
				if (doc && doc.exists) {
					...
					if (docData) {
						...
						const postDocRef = offChainCollRef.doc(
							String(docData.id),
						);
			...
			const topicFromType = getTopicFromType(
			...
			const subsquidPostsPromise = subsquidPosts?.map(
				async (subsquidPost): Promise<IPostListing> => {
					const {
						createdAt,
						end,
						hash,
					...
					const postId =
						proposalType === ProposalType.TIPS ? hash : index;
			...
			if (proposalType === ProposalType.OPEN_GOV) {
				...
				strProposalType === ProposalType.FELLOWSHIP_REFERENDUMS
			) {
				if (
			...
			if (
				...
				if (proposalType === ProposalType.ANNOUNCEMENT) {
						...
						if (postDoc && postDoc.exists) {
							...
							if (data) {
								...
								if (
									...
									const res =
										await getSubSquareContentAndTitle(
											strProposalType,
											network,
											postId,
										);
								...
								return {
									cid: cid,
									comments_count:
										commentsQuerySnapshot.data()?.count ||
										0,
									created_at: createdAt,
									gov_type: data.gov_type,
									hash,
					...
					postsPromise = subsquidPosts?.map(async (subsquidPost) => {
						...
						const title = splitterAndCapitalizer(
							subsquidPost.callData?.method || '',
							'_',
						);
						...
						if (postDoc && postDoc.exists) {
							...
							if (data) {
								...
								return {
									...
									tags: data?.tags || [],
									title:
										data?.title || subsquareTitle || title,
									type: type || subsquidProposalType,
									user_id: data?.user_id || 1,
								};
							}
				...
				postsPromise = subsquidPosts?.map(
					async (subsquidPost): Promise<IPostListing> => {
						const {
							createdAt,
							end,
						...
						let otherPostProposer = '';
						const method = splitterAndCapitalizer(
							subsquidPost.callData?.method || '',
							'_',
						);
						if (group?.proposals?.length) {
							...
							if (data) {
								...
								return {
									...
									proposer:
										...
										curator,
									requestedAmount:
										preimage?.proposedCall?.args?.amount ||
										preimage?.proposedCall?.args?.value ||
										null,
									status,
									tags: data?.tags || [],
									title: data?.title || subsquareTitle,
			...
			const data: IPostsListingResponse = {
				count: Number(
					subsquidData?.proposalsConnection?.totalCount ||
						subsquidData?.announcementsConnection?.totalCount ||
						0,
				),
				posts,
			};
...
export const getSpamUsersCountForPosts = async (
	network: string,
	posts: any[],
	proposalType?: string,
): Promise<any[]> => {
		...
		for (let i = 0; i < newIdsLen; i += 30) {
			...
			let querySnapshot = networkDocRef(network)
				...
				.where(
					'content_id',
					'in',
					postsIds.slice(i, newIdsLen > i + 30 ? i + 30 : newIdsLen),
				)
				.get();
			if (proposalType) {
				...
				if (doc && doc.exists) {
					...
					if (index !== undefined && index !== null) {
						if (
							...
							(proposalType ||
								data.proposal_type ===
									getFirestoreProposalType(posts[index].type))
						) {
							posts[index] = {
								...posts[index],
								spam_users_count:
									Number(posts[index].spam_users_count || 0) +
									1,
							};
						}
					}
