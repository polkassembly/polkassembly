...
export async function getPostsByAddress(
  ...
  try {
    ...
    const proposalsPromise = (
      subsquidData?.proposalsConnection?.edges as any[]
    )?.map(async (edge) => {
      if (edge) {
        ...
        if (node) {
          ...
          const key = firestoreProposalType.replace('_proposals', '') as
            | 'democracy'
            | 'treasury';
          const proposalDocSnapshot = await postTypesColRef
            .doc(firestoreProposalType)
            .collection('posts')
            .doc(String(node.index))
            .get();
...
const handler: NextApiHandler<
  IPostsByAddressListingResponse | { error: string }
> = async (req, res) => {
