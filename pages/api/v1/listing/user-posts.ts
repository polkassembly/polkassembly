...
export const getUserPosts: TGetUserPosts = async (params) => {
    try {
        ...
        const discussionsQuerySnapshot = await postsByTypeRef(
            ...
            .where("user_id", "==", numUserId)
            .get();
        const discussionsPromise = discussionsQuerySnapshot.docs.map(
            async (doc) => {
                const data = doc.data();
            ...
            const onChainPostsPromise = edges?.map(async (edge) => {
                if (edge && edge.node) {
                    ...
                    const newData: IUserPost = {
                        content:
                            description ||
                            (proposalArguments && proposalArguments.description
