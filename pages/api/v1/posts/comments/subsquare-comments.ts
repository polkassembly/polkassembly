...
const urlMapper: any = {
	[ProposalType.BOUNTIES]: (id: any, network: string) =>
		`https://${network}.subsquare.io/api/treasury/bounties/${id}/comments`,
	[ProposalType.CHILD_BOUNTIES]: (id: any, network: string) =>
		`https://${network}.subsquare.io/api/treasury/child-bounties/${id}/comments`,
	[ProposalType.COUNCIL_MOTIONS]: (id: any, network: string) =>
		`https://${network}.subsquare.io/api/motions/${id}/comments`,
	[ProposalType.DEMOCRACY_PROPOSALS]: (id: any, network: string) =>
		`https://${network}.subsquare.io/api/democracy/proposals/${id}/comments`,
	[ProposalType.FELLOWSHIP_REFERENDUMS]: (id: any, network: string) =>
		`https://${network}.subsquare.io/api/fellowship/referenda/${id}/comments`,
	[ProposalType.REFERENDUMS]: (id: any, network: string) =>
		`https://${network}.subsquare.io/api/democracy/referendums/${id}/comments`,
	[ProposalType.REFERENDUM_V2]: (id: any, network: string) =>
		`https://${network}.subsquare.io/api/gov2/referendums/${id}/comments`,
	[ProposalType.TECH_COMMITTEE_PROPOSALS]: (id: any, network: string) =>
		`https://${network}.subsquare.io/api/tech-comm/motions/${id}/comments`,
...
const extractContent = async (markdownContent: string, network: any) => {
	...
	if (matches) {
		const promises = matches.map(async (match) => {
			const [label, addressWithNetwork] = match
