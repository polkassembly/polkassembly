...
export const getUpdatedAt = (data: any) => {
  ...
  if (data) {
    if (data.last_edited_at) {
      updated_at = data.last_edited_at?.toDate
        ? data.last_edited_at.toDate()
        : data.last_edited_at;
...
export async function getOffChainPost(
  ...
  try {
    ...
    const spam_users_count = await getSpamUsersCount(
      network,
      proposalType,
      Number(postId),
      'post',
    ...
    const post: IPostResponse = {
      ...
      type:
        strProposalType === 'discussions'
          ? 'Discussions'
          : strProposalType === 'grants'
          ? 'Grants'
          : '',
    ...
    if (post && (post.user_id || post.user_id === 0)) {
      ...
      const addressDocs = await firestore_db
      ...
      if (isProposalTypeValid(type)) {
        ...
        if (!post_link.description) {
          post_link.description =
            postData.description || preimage?.proposedCall?.description;
        }
