...
const getAndSetNewData = async (params: IParams) => {
  ...
  if (
    (!data?.title ||
      !data.content ||
      isDefaultStringExist(data.content, proposalType.toString())) &&
    timeline &&
            ...
            if (data) {
              ...
              if (!newData.proposer_address) {
                newData.proposer_address =
                  getProposerAddressFromFirestorePostData(data, network);
              }
              ...
              if (!newData.topic_id) {
                newData.topic_id =
                  getTopicFromFirestoreData(data, proposalType)?.id || null;
              }
        ...
        if (colDocNameArr.length >= 6) {
          ...
          const dummyData = value.data
            ? {
                ...
                content:
                  value.data.content &&
                  !isDefaultStringExist(value?.data?.content, pathPostType)
                    ? value.data.content
                    : newData.content,
...
export async function getComments(
  ...
  const commentsPromise = commentsSnapshot.docs.map(async (doc) => {
    if (doc && doc.exists) {
      ...
      const history = data?.history
        ...
        : [];
      const commentDocRef = postDocRef
        .collection('comments')
        .doc(String(doc.id));
      const commentsReactionsSnapshot = await commentDocRef
      ...
      const repliesSnapshot = await commentDocRef
        .collection('replies')
      ...
      repliesSnapshot.docs.forEach((doc) => {
        if (doc && doc.exists) {
          const data = doc.data();
          if (data) {
            const { created_at, id, username, comment_id, content, user_id } =
              data;
            if (id) {
              replyIds.push(id);
            }
  ...
  if (newIds.length > 0) {
    ...
    for (let i = 0; i < totalChunks; i++) {
      ...
      if (slice.length > 0) {
        const usersQuery = await firestore_db
          .collection('users')
          .where('id', 'in', slice)
          .get();
        usersQuery.docs.forEach((doc) => {
          if (doc && doc.exists) {
            ...
            userIdToUserMap[data.id] = {
              is_custom_username:
                MANUAL_USERNAME_25_CHAR.includes(data.username) ||
        ...
        addressesQuery.docs.forEach((doc) => {
          if (doc && doc.exists) {
            ...
            userIdToUserMap[data.user_id] = {
              is_custom_username:
                userIdToUserMap?.[data.user_id]?.is_custom_username,
  ...
  return comments.map((comment) => {
    if (comment && userIdToUserMap[comment?.user_id]) {
      comment.proposer =
        userIdToUserMap[comment.user_id].proposer || comment.proposer;
      comment.username =
        userIdToUserMap[comment.user_id].username || comment.username;
      comment.is_custom_username =
        userIdToUserMap[comment.user_id].is_custom_username;
      if (
        comment.replies &&
        ...
        comment.replies = comment.replies.map((reply) => {
          if (reply && userIdToUserMap[reply?.user_id]) {
            reply.proposer =
              userIdToUserMap[reply.user_id].proposer || reply.proposer;
            reply.username =
              userIdToUserMap[reply.user_id].username || reply.username;
            reply.is_custom_username =
              userIdToUserMap[reply.user_id].is_custom_username;
          }
          return reply;
        });
      }
...
export async function getOnChainPost(
  ...
  try {
    const { network, postId, voterAddress, proposalType, isExternalApiCall } =
      params;
    ...
    const postData =
      subsquidData.proposals?.[0] || subsquidData.announcements?.[0];
    ...
    let proposer =
      postData?.proposer || preimage?.proposer || postData?.curator;
    if (
      !proposer &&
      (postData?.parentBountyIndex || postData?.parentBountyIndex === 0)
    ) {
      const subsquidRes = await fetchSubsquid({
        network,
          ...
          subsquidPosts.forEach((post) => {
            if (postData?.parentBountyIndex === post.index && post) {
              proposer =
                post.proposer ||
                post.curator ||
                (post?.preimage ? post?.preimage?.proposer : '');
            }
          });
        }
      }
    ...
    const post: IPostResponse = {
      ...
      method:
        preimage?.method || proposedCall?.method || proposalArguments?.method,
    ...
    if (
      ['referendums', 'open_gov'].includes(strProposalType) &&
      voterAddress &&
      postData?.votes?.[0]?.decision
    ) {
    ...
    if (proposalType === ProposalType.TREASURY_PROPOSALS) {
      post.deadline = null;
      const eventSnapshot = await netDocRef
        .collection('events')
        .where('post_id', '==', strPostId)
        .limit(1)
        .get();
    ...
    if (proposalType === ProposalType.TIPS) {
      post.tippers =
        subsquidData?.tippersConnection?.edges?.reduce(
          (tippers: any[], edge: any) => {
            if (edge && edge?.node) {
    ...
    if (proposalType === ProposalType.COUNCIL_MOTIONS) {
      post.motion_votes =
        subsquidData?.votesConnection?.edges?.reduce(
          (motion_votes: any[], edge: any) => {
            if (edge && edge?.node) {
    ...
    if (proposalType === ProposalType.BOUNTIES) {
      post.child_bounties_count =
        subsquidData?.proposalsConnection?.totalCount || 0;
      post.child_bounties =
        subsquidData?.proposalsConnection?.edges?.reduce(
          (child_bounties: any[], edge: any) => {
            if (edge && edge?.node) {
      ...
      if (data && post) {
        ...
        if (post_link) {
          ...
          if (postDoc.exists && postData) {
            ...
            post_link.created_at = postData?.created_at?.toDate
    ...
    if (
      ...
      const commentPromises = post.timeline.map(async (timeline: any) => {
        ...
        ).doc(String(timeline.type === 'Tip' ? timeline.hash : timeline.index));
      ...
      if (post.post_link) {
        ...
        const postDocRef = postsByTypeRef(network, type).doc(String(id));
        const commentsSnapshot = await postDocRef.collection('comments').get();
        post.comments = await getComments(
          commentsSnapshot,
          postDocRef,
          network,
          strProposalType,
    ...
    if (
      ...
      post.title =
        splitterAndCapitalizer(postData?.callData?.method || '', '_') ||
        postData?.cid;
    }
...
export const getSpamUsersCount = async (
  network: string,
  proposalType: any,
