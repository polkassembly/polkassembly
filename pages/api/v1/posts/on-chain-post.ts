                ...
                if (colDocNameArr.length >= 6) {
                    ...
                    if (dummyData) {
                        ...
                        if (!realData.proposer_address && proposer) {
                            realData.proposer_address =
                                getSubstrateAddress(proposer);
                        }
            ...
            if (
                ...
                comment.replies = comment.replies.map((reply) => {
                    if (reply && userIdToUserMap[reply?.user_id]) {
                        reply.proposer =
                            userIdToUserMap[reply.user_id].proposer ||
                            reply.proposer;
                        reply.username =
                            userIdToUserMap[reply.user_id].username ||
                            reply.username;
...
export async function getOnChainPost(
    ...
    try {
        ...
        const proposalArguments =
            postData?.proposalArguments || postData?.callData;
            ...
            if (subsquidRes && subsquidRes?.data) {
                ...
                if (
                    ...
                    subsquidPosts.forEach((post) => {
                        if (
                            postData?.parentBountyIndex === post.index &&
                            post
                        ) {
                            proposer =
                                post.proposer ||
