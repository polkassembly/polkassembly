...
const urlMapper = {
  [ProposalType.BOUNTIES]: (id: number | string, network: string) =>
    `https://${network}.subsquare.io/api/treasury/bounties/${id}`,
  [ProposalType.CHILD_BOUNTIES]: (id: number | string, network: string) =>
    `https://${network}.subsquare.io/api/treasury/child-bounties/${id}`,
  [ProposalType.COUNCIL_MOTIONS]: (id: number | string, network: string) =>
    `https://${network}.subsquare.io/api/motions/${id}`,
  [ProposalType.DEMOCRACY_PROPOSALS]: (id: number | string, network: string) =>
  ...
  [ProposalType.REFERENDUM_V2]: (id: number | string, network: string) =>
...
export const getSubSquareContentAndTitle = async (
  ...
  try {
    ...
    subsqTitle =
      subsqTitle.length !== 0
        ? String(data?.title)?.includes('Untitled') || data?.title
        : '';
...
const handler: NextApiHandler<
  | { data: { content: any | string; title: string | any } | undefined }
  | { error: string }
> = async (req, res) => {
