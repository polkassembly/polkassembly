...
async function handler(
  ...
  if (!voteTypes.includes(strVoteType)) {
    return res
      .status(400)
      .json({ error: `The voteType "${voteType}" is invalid.` });
  }
  ...
  if (!isVotesSortOptionsValid(strSortBy)) {
    return res
      .status(400)
      .json({ error: `The sortBy "${sortBy}" is invalid.` });
  }
  const variables: any = {
    index_eq: numPostId,
    ...
    orderBy:
      strSortBy === votesSortValues.BALANCE
  ...
  promiseResults.forEach((result, i) => {
    ...
    if (result && result.status === 'fulfilled' && result.value) {
      ...
      if (voteType === VoteType.REFERENDUM_V2) {
        ...
        resObj[decision].count =
          subsquidData?.convictionVotesConnection?.totalCount;
      }
    }
  });
