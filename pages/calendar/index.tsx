...
const CalendarView: FC<ICalendarViewProps> = ({
        ...
        scheduledEvents.forEach((eventObj, i) => {
          eventsArr.push({
            content: eventObj?.data?.id
              ? `Execute named scheduled task ${eventObj?.data?.id}`
  ...
  if (
    ...
    allowed_roles.includes(ALLOWED_ROLE)
  ) {
    accessible = true;
  }
  ...
  const handleUpdateApproval = async () => {
    if (
      !sidebarEvent ||
      !eventApprovalStatus ||
      Object.keys(sidebarEvent).length === 0
    ) {
      return;
    }
  ...
  const EventWrapperComponent = ({ event, children }: any) => {
    ...
    const statusClassName = dayjs(event.end_time).isBefore(new Date())
      ? 'overdue-border'
      : `${event.status?.toLowerCase()}-border`;
      ...
      <div className={`${className} bg-white drop-shadow-md p-3 rounded-xl`}>
        ...
        {accessible && (
          <div className="event-bot-div">
            ...
            >
              {queryApprovalStatus == approvalStatus.APPROVED ? 'Show' : 'Hide'}{' '}
              Pending Events
            </Button>
          </div>
        )}
              ...
              >
                <div className="p-5 pl-2 pt-0">
                  <p className="text-sidebarBlue font-medium text-md text-center mb-2">
                    Current Time:{' '}
                    {dayjs(utcDate).format('D-MM-YY | h:mm a UTC')}{' '}
                  </p>
                  ...
                  <div className="font-medium text-md text-sidebarBlue mb-3">
                    Proposal Status:{' '}
                  </div>
                  ...
                  <div className="font-medium text-md text-sidebarBlue mt-8 mb-3">
                    Categories:{' '}
                  </div>
                  <Checkbox.Group
        ...
        >
          <div className="events-sidebar">
              ...
              <div className="d-flex">
                <div
                  className={`status-icon ${
                    dayjs(sidebarEvent.end_time).isBefore(new Date())
                      ? 'overdue-color'
            ...
            <div className="sidebar-event-datetime">
              <span>{dayjs(sidebarEvent.end_time).format('MMMM D')}</span>{' '}
              <span>{dayjs(sidebarEvent.end_time).format('h:mm a')}</span>
            </div>

            {sidebarEvent.content && (
              <div className="sidebar-event-content">
                {`${sidebarEvent.content.substring(0, 769)} ${
                  sidebarEvent.content.length > 769 ? '...' : ''
                }`}
                {sidebarEvent.content.length > 769 && (
                  <>
                    <br />
                    <a href={sidebarEvent.url} target="_blank" rel="noreferrer">
                      Show More
                    </a>
                  </>
                )}
            ...
            <div className="sidebar-event-links">
              <h3 className="dashboard-heading flex items-center gap-x-2 mb-2">
                {' '}
                <Image alt="link" src={chainLink} height={16} width={16} />{' '}
                Relevant Links
              </h3>
