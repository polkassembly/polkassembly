...
export const getServerSideProps: GetServerSideProps = async ({
	...
	if (!networkTrackInfo[network][PostOrigin.STAKING_ADMIN]) {
		return { props: { error: `Invalid track for ${network}` } };
	}

	const { trackId } = networkTrackInfo[network][PostOrigin.STAKING_ADMIN];
	...
	const props: IStakingAdminProps = {
		network,
		posts: {},
	};
...
const StakingAdmin: FC<IStakingAdminProps> = (props) => {
	...
	return (
		<>
			<SEOHead
				title={PostOrigin.STAKING_ADMIN.split(/(?=[A-Z])/).join(' ')}
				network={network}
			/>
			<TrackListing trackName={PostOrigin.STAKING_ADMIN} posts={posts} />
		</>
	);
};
