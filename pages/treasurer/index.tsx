	...
	if (!trackStatus && !filterBy) {
		if (queryNetwork) {
			return {
				props: {},
				redirect: {
					destination: `/treasurer?trackStatus=all&page=1&network=${network}`
				}
			};
		} else {
			return {
				props: {},
				redirect: {
					destination: '/treasurer?trackStatus=all&page=1'
				}
			};
		}
	}
...
const Treasurer: FC<ITreasurerProps> = (props) => {
