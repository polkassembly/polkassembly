...
export const getServerSideProps: GetServerSideProps = async ({ req, query, locale }) => {
	...
	if (!trackStatus && !filterBy) {
		return {
			props: {},
			redirect: {
				destination: '/wish-for-change?trackStatus=all&page=1'
			}
		};
	}

	if (!networkTrackInfo[network][PostOrigin.WISH_FOR_CHANGE]) {
		return { props: { error: `Invalid track for ${network}` } };
	}

	const { trackId } = networkTrackInfo[network][PostOrigin.WISH_FOR_CHANGE];
	...
	const props: IWishForChangeProps = {
		network,
		posts: {},
		...translations
	};
...
interface IWishForChangeProps {
	posts: IReferendumV2PostsByStatus;
	network: string;
	error?: string;
}

const WishForChange: FC<IWishForChangeProps> = (props) => {
	...
	return (
		<>
			<SEOHead
				title={PostOrigin.WISH_FOR_CHANGE.split(/(?=[A-Z])/).join(' ')}
				network={network}
			/>
			<TrackListing
