...
export async function _processIdentityVerificationSignUp({ network, address }: { network: string; address: string }) {
	console.log('Processing identity verification sign up', network, address);
	...
	await changeProfileScoreForAddress(address, REPUTATION_SCORES.on_chain_identity_verification_sign_up.value);
}
