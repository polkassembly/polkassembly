...
const privateKey =
  process.env.NODE_ENV === 'test'
    ? process.env.JWT_PRIVATE_KEY_TEST
    : process.env.JWT_PRIVATE_KEY?.replace(/\\n/gm, '\n');
const jwtPublicKey =
  process.env.NODE_ENV === 'test'
...
const passphrase =
  process.env.NODE_ENV === 'test'
    ? process.env.JWT_KEY_PASSPHRASE_TEST
...
export const getAddressSignupKey = (address: string): string =>
  `ASU-${address}`;
export const getSetCredentialsKey = (address: string): string =>
  `SCR-${address}`;
export const getEmailVerificationTokenKey = (token: string): string =>
  `EVT-${token}`;
export const getMultisigAddressKey = (address: string): string =>
  `MLA-${address}`;
...
class AuthService {
  ...
  private async getLatestUserCount(): Promise<number> {
    try {
      const userCount = await firebaseAdmin
        .firestore()
        .collection('users')
        .orderBy('id', 'desc')
        .limit(1)
  ...
  public async Login(
    ...
    if (!isEmail) {
      for (let i = 0; i < nameBlacklist.length; i++) {
        if (username.toLowerCase().includes(nameBlacklist[i]))
          throw apiErrorWithStatusCode(messages.USERNAME_BANNED, 401);
      }
    }
    ...
    if (isEmail) {
      userQuery = await collection
        ...
        .get();
      if (userQuery.size === 0)
        throw apiErrorWithStatusCode(messages.NO_USER_FOUND_WITH_EMAIL, 404);
    } else {
      userQuery = await collection
        ...
        .get();
      if (userQuery.size === 0)
        throw apiErrorWithStatusCode(messages.NO_USER_FOUND_WITH_USERNAME, 404);
    }
  ...
  public async SetDefaultAddress(
    ...
    newAddresses.forEach((addressObj) => {
      const addressRef = firebaseAdmin
        .firestore()
        .collection('addresses')
        .doc(addressObj.address);
  ...
  public async AddressLogin(
    ...
    if (!user)
      throw apiErrorWithStatusCode(messages.ADDRESS_LOGIN_NOT_FOUND, 404);
  ...
  public async AddressSignupStart(address: string): Promise<string> {
    const addressDoc = await firebaseAdmin
      ...
      .get();
    if (addressDoc.exists)
      throw apiErrorWithStatusCode(messages.ADDRESS_SIGNUP_ALREADY_EXISTS, 401);
  ...
  public async LinkProxyAddress(
    ...
    const alreadyExists = (
      await firestore.collection('addresses').doc(proxied).get()
    ).exists;
    if (alreadyExists)
      apiErrorWithStatusCode(
  ...
  public async AddressSignupConfirm(
    network: Network,
    address: string,
    signature: string,
    wallet: Wallet,
    ...
    const addressDoc = await firebaseAdmin
      ...
      .get();
    if (addressDoc.exists)
      throw apiErrorWithStatusCode(messages.ADDRESS_SIGNUP_ALREADY_EXISTS, 400);
  ...
  public async SignUp(
    ...
    for (let i = 0; i < nameBlacklist.length; i++) {
      if (username.toLowerCase().includes(nameBlacklist[i]))
        throw apiErrorWithStatusCode(messages.USERNAME_BANNED, 400);
    }
  ...
  public async AddressUnlink(token: string, address: string): Promise<string> {
    ...
    const addressDoc = await firestore
      ...
      .get();
    if (addressDoc.empty)
      throw apiErrorWithStatusCode(messages.ADDRESS_NOT_FOUND, 400);

    const addressObj = addressDoc.docs[0].data() as Address;
    if (addressObj.default)
      throw apiErrorWithStatusCode(messages.ADDRESS_UNLINK_NOT_ALLOWED, 403);
  ...
  public async AddressLinkConfirm(
    token: string,
    address: string,
    signature: string,
    wallet: Wallet,
    ...
    const addressToLinkDoc = await firestore
      .collection('addresses')
      .doc(address)
      .get();
    if (!addressToLinkDoc.exists)
      throw apiErrorWithStatusCode(messages.ADDRESS_NOT_FOUND, 404);
  ...
  public async MultiSigAddressLinkConfirm(
    ...
    if (!signMessage)
      throw apiErrorWithStatusCode(messages.MULTISIG_SIGN_MESSAGE_EXPIRED, 403);
    ...
    if (!signatories.includes(signatory))
      throw apiErrorWithStatusCode(messages.MULTISIG_NOT_ALLOWED, 400);

    const isValidSr = verifySignature(signMessage, signatory, signature);
    if (!isValidSr)
      throw apiErrorWithStatusCode(messages.ADDRESS_LINKING_FAILED, 403);
  ...
  public async ChangeNotificationPreference(
    ...
    const userPreferenceRef = networkDocRef(network)
      .collection('user_preferences')
      .doc(String(userId));
      ...
      return {
        new_proposal:
          new_proposal === undefined
            ? notification_preferences.new_proposal
            : new_proposal,
        own_proposal:
          own_proposal === undefined
            ? notification_preferences.own_proposal
            : own_proposal,
        post_created:
          post_created === undefined
            ? notification_preferences.post_created
            : post_created,
        post_participated:
    ...
    if (userPreferenceSnapshot.exists) {
      const { notification_preferences } =
        userPreferenceSnapshot.data() as IUserPreference;
  ...
  public async resendVerifyEmailToken(
    ...
    const user = await getUserFromUserId(userId);
    if (!user.email)
      throw apiErrorWithStatusCode(messages.EMAIL_NOT_FOUND, 404);
  ...
  public async SetCredentialsConfirm(
    address: string,
    email: string,
    newPassword: string,
    signature: string,
    ...
    const userEmailQuerySnapshot = await firestore
      .collection('users')
  ...
  public async ChangeEmail(
    ...
    const existingUndoTokenDoc = await firestore
    ...
    if (
      existingUndoTokenDoc.size > 0 &&
      existingUndoTokenDoc.docs[0].data().valid
    ) {
      const now = dayjs();
  ...
  public async ResetPassword(
    ...
    if (!isValid)
      throw apiErrorWithStatusCode(messages.PASSWORD_RESET_TOKEN_INVALID, 403);
  ...
  public async UndoEmailChange(
    ...
    if (undoTokenSnapshot.empty)
      throw apiErrorWithStatusCode(messages.EMAIL_UNDO_TOKEN_NOT_FOUND, 400);

    const undoToken = undoTokenSnapshot.docs[0].data() as UndoEmailChangeToken;
    if (!undoToken.valid)
      throw apiErrorWithStatusCode(messages.INVALID_EMAIL_UNDO_TOKEN, 403);
  ...
  public async CreatePostConfirm(
    ...
    if (!isValidSr)
      throw apiErrorWithStatusCode(messages.POST_CREATE_INVALID_SIGNATURE, 403);
    ...
    const postsCount =
      (await offChainPostCollectionRef.count().get()).data().count || 0;
  ...
  public async EditPostConfirm(
    ...
    const isValidSr = verifySignature(signContent, address, signature);
    if (!isValidSr)
      throw apiErrorWithStatusCode(messages.POST_EDIT_INVALID_SIGNATURE, 400);
  ...
  public async ProposalTrackerCreate(
    ...
    const firestore = firebaseAdmin.firestore();
    const newEventDocRef = firestore
      .collection('networks')
      .doc(network)
      .collection('events')
      .doc();
  ...
  public async ProposalTrackerUpdate(
    ...
    const proposalEventDoc = await firestore
      .collection('proposal_trackers')
      .doc(id)
      .get();
    if (!proposalEventDoc.exists)
      throw apiErrorWithStatusCode(messages.ERROR_IN_PROPOSAL_TRACKER, 400);
  ...
  public async SendVerifyEmail(
    ...
    const shouldSendUndoEmailChangeEmail = !oldMail
      ? false
      : oldMail !== email
      ? true
      : false;
