...
const getIdentityInformation = async ({ api, apiReady, address, network }: Args): Promise<IIdentityInfo> => {
	...
	return {
		...
		display: isHex(identity?.display?.Raw) ? hexToString(identity?.display?.Raw) : identity?.display?.Raw || '',
		...
		legal: isHex(identity?.legal?.Raw) ? hexToString(identity?.legal?.Raw) : identity?.legal?.Raw || '',
