...
const GovernanceSwitchButton = ({ className, previousRoute }: Props) => {
    ...
    return (
        <div className={`${className}`}>
            <Link
                className="m-0 border border-solid border-[rgba(72,95,125,0.2)] rounded-full flex items-center justify-center min-w-[200px] p-2 py-1"
                href={isGov2Route ? "/" : "/opengov"}
            >
                <p className="text-sidebarBlue font-normal text-[10px] leading-[15px] tracking-[0.02em] m-0">
                    Switch to
                </p>
                <p className="font-poppins ml-[6px] mr-[11px] text-sidebarBlue text-xs font-semibold leading-[18px] tracking-[0.02em] m-0">
                    {" "}
                    {isGov2Route ? "Governance V1" : "OpenGov"}{" "}
                </p>
                <p className="flex items-center justify-center text-navBlue text-base m-0">
                    <SyncIcon />
                </p>
            </Link>
        </div>
