...
const NavHeader = ({
	...
	return (
		<Header
			className={`${className} shadow-md z-[1001] sticky top-0 flex items-center bg-white h-[60px] max-h-[60px] px-6 leading-normal border-solid border-t-0 border-r-0 border-b-2 border-l-0 border-pink_primary`}
		>
			<MenuOutlined
				...
				<div className="flex items-center justify-between gap-x-2 md:gap-x-4">
					...
					<Space className="hidden md:flex items-center justify-between gap-x-2 md:gap-x-4">
						...
						{!username && (
							<div className="flex items-center lg:gap-x-2">
								<Button
									className="w-[60px] h-[22px] lg:w-[74px] lg:h-[32px] bg-pink_primary rounded-[2px] md:rounded-[4px] text-white lg:text-sm lg:font-medium lg:leading-[21px] tracking-[0.00125em] flex items-center justify-center hover:text-white"
									onClick={() => {
										setSidedrawer(false);
										setLoginOpen(true);
									}}
					...
					{open ? (
						...
						>
							...
							<div
								className={`absolute w-screen bg-black bg-opacity-50 top-[60px] left-0 overflow-hidden h-[calc(100vh-60px)] ${
									!sidedrawer && open ? 'block' : 'hidden'
								}`}
							>
								<div
									onClick={() => {
								...
								>
									<div className="flex flex-col">
										<SearchBar />
										<div>
											<p className="m-0 p-0 text-[#485F7D] font-normal text-sm leading-[23px] tracking-[0.02em] text-left">
												Network
											</p>
											<NetworkDropdown
												setSidedrawer={() => {}}
