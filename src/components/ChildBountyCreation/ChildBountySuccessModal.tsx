			...
			<div className='flex flex-col items-center justify-center'>
				<div className='-mt-[232px]'>
					<Image
						...
						alt={t('success_delegate_icon')}
						width={363}
						height={347}
					/>
				</div>

				<label className='mt-0 text-xl font-semibold text-bodyBlue dark:text-blue-dark-high'>{t('child_bounty_creation_initiated')}</label>
				...
				<div className='my-2 flex'>
					<div className='mt-[10px] flex flex-col gap-1.5 text-sm text-lightBlue dark:text-blue-dark-medium'>
						<span className='flex'>
							<span className='w-[172px]'>{t('proposer')}:</span>
							<div className='flex gap-3'>
								<Address
									...
									disableTooltip
								/>
								{multisigData?.threshold > 0 && <Tag className={'rounded-full bg-[#EFF0F1] px-3 py-0.5 text-xs text-[#F4970B] dark:bg-[#EFF0F1]'}>{t('multisig_address')}</Tag>}
							</div>
						</span>
						{!!curator?.length && (
							<span className='flex'>
								<span className='w-[172px]'>{t('child_bounty_curator')}:</span>
						...
						{!bnRequestedAmount?.eq(ZERO_BN) && (
							<span className='flex'>
								<span className='w-[172px]'>{t('child_bounty_amount')}:</span>
						...
						{parentBountyIndex && !isNaN(parentBountyIndex) && (
							<span className='flex'>
								<span className='w-[172px]'>{t('link_to_parent_bounty')}:</span>
