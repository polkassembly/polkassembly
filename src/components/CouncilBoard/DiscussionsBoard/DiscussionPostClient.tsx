...
const DiscussionPostClient: FC<IDiscussionPostClient> = ({ councilBoardSidebar = false, postID }) => {
	const [loading, setLoading] = useState(true);
	const [error, setError] = useState('');
	const [post, setPost] = useState<IPostResponse>();
	useEffect(() => {
		setLoading(true);
		nextApiClientFetch<IPostResponse>(`api/v1/posts/discussion?postId=${postID}`)
			.then((res) => {
				if (res.data) {
					setPost(res.data);
				} else if (res.error) {
