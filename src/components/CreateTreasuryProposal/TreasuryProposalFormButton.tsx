...
const TreasuryProposalFormButton = ({
  ...
  const minimumBond =
    chainProperties[network]?.tokenSymbol === 'DOT'
      ? '100.0000 DOT'
      : '66.66 mKSM';
  ...
  const handleDetect = async (updateForInput: AvailableAccountsInput) => {
    ...
    switch (updateForInput) {
      ...
      case AvailableAccountsInput.beneficiary:
        availableAccountsObj.beneficiary =
          !showAvailableAccountsObj['beneficiary'];
        break;
    }
          ...
          return (
            address && (
              <div
                key={address}
                onClick={() =>
                  handleSelectAvailableAccount(updateForInput, address)
                }
  ...
  const onPostDescriptionChange = (data: string) => {
  ...
  const saveProposal = async (
    ...
    const proposalId: number = (
      (await api.query.treasury.proposalCount()) as any
    ).toNumber();

    const { data, error: apiError } =
      await nextApiClientFetch<CreatePostResponseType>(
        'api/v1/auth/actions/createTreasuryPost',
        {
          content,
          postId: proposalId,
          proposerAddress,
  ...
  const triggerBtn = (
    <button
      disabled={!id}
      className="outline-none whitespace-pre border-none p-3 font-medium  leading-[20px] tracking-[0.01em] shadow-[0px_6px_18px_rgba(0,0,0,0.06)] flex items-center justify-center rounded-[4px] text-white bg-pink_primary cursor-pointer -mt-1"
  ...
  const triggerBtnLoginDisabled = (
    <Tooltip
      color="#E5007A"
      title="Please signup/login to create treasury proposal"
    >
      {' '}
      <button
        disabled={true}
        className="font-medium text-sm p-4 rounded-[4px] shadow-[0px_6px_18px_rgba(0,0,0,0.06)] text-white border-none outline-none cursor-pointer  bg-grey_secondary"
      >
        + Add New Proposal
      </button>
    </Tooltip>
          ...
          <div className="modal-desc">
            <Form className="identity-form">
              <div>
                ...
                <div className=" mb-[1.5em]">
                  <label className="mb-3 font-bold flex items-center text-sm text-sidebarBlue">
                    ...
                    <HelperTooltip
                      className="ml-2"
                      text="This account will make the proposal and be responsible for the bond."
                    />
                  </label>
                  ...
                  <div className=" flex items-center">
                    ...
                    <Form.Item
                      ...
                      validateStatus={
                        errorsFound.includes('submitWithAccount') ? 'error' : ''
                      }
                    >
                      {errorsFound.includes('submitWithAccount') && (
                  ...
                  {!extensionNotAvailable && (
                    <div className=" flex justify-between mb-[1em]">
                      <div
                        onClick={() =>
                          handleDetect(AvailableAccountsInput.submitWithAccount)
                        }
                ...
                <>
                  <div className="mb-[1.5em]">
                    <label className=" mb-3 font-bold flex items-center text-sm text-sidebarBlue">
                      ...
                      <HelperTooltip
                        className="ml-2"
                        text="The beneficiary will receive the full amount if the proposal passes."
                      />
                    </label>
                    ...
                    <div className=" flex items-center">
                      ...
                      <Form.Item
                        className=" mb-0 w-full"
                        validateStatus={
                          errorsFound.includes('beneficiaryAccount')
                            ? 'error'
                            : ''
                        }
                      >
                    ...
                    {!extensionNotAvailable && (
                      <div className=" flex justify-between mb-[1em]">
                        <div
                          onClick={() =>
                            handleDetect(AvailableAccountsInput.beneficiary)
                          }
                          className=" text-pink_primary cursor-pointer ml-[1.5em] mt-[0.25em]"
                        >
                          or choose from available addresses
                    ...
                    {showAvailableAccountsObj['beneficiary'] &&
                      availableAccounts.length > 0 &&
                      getAvailableAccounts(AvailableAccountsInput.beneficiary)}
                  </div>
                </>
                ...
                {treasuryProposal.minBond ? (
                  <div className="mb-[1.5em]">
                    ...
                    <Input className=" hide-pointer" value={minimumBond} />
                  </div>
                ) : null}

                <p>
                  <WarningFilled /> Be aware that once submitted the proposal
                  will be put to a council vote. If the proposal is rejected due
                  to a lack of info, invalid requirements or non-benefit to the
                  network as a whole, the full bond posted (as describe above)
                  will be lost.
                </p>
              </div>
              ...
              <div>
                ...
                <ContentForm onChange={onPostDescriptionChange} />
              </div>
            </Form>
          </div>
        </div>
