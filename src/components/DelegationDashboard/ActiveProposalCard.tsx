...
const ActiveProposalCard = ({
	...
	return (
		<Link href={`/referenda/${proposal?.post_id}`}>
			<div
				className={
					'border-[#D2D8E0] border-solid border-[1px] rounded-t-[6px] hover:border-[#E5007A] rounded-[6px]'
				}
			>
				<div className="px-6 py-6 border-[1px] flex justify-between max-sm:gap-2 max-sm:items-start max-sm:flex-col hover:border-pink_primary">
                  ...
                  By:
					...
					<Button
						className={`flex justify-center mt-2 gap-2 border-none bg-white shadow-none ${
							status.includes(ETrackDelegationStatus.Delegated) && 'opacity-50'
						}`}
						disabled={status.includes(ETrackDelegationStatus.Delegated)}
					>
						<VoteIcon />
						<span className="text-pink_primary text-sm font-medium">
              Cast Vote
						</span>
					</Button>
				</div>
        ...
        isAbstain ? (
						<div
							className={`border-solid py-2 px-6 flex gap-2 border-[1px] rounded-b-[5px] ${
								isAye && 'bg-[#F0FCF6] border-[#2ED47A]'
							} ${isNay && 'bg-[#fff1f4] border-[#FF3C5F]'} ${
								isAbstain && 'bg-[#f9f9f9] border-[#ABABAC]'
							}`}
						>
							{status.includes(ETrackDelegationStatus.Delegated) && (
								<Address address={String(delegatedTo)} displayInline />
							)}
              ...
              Voted:
                  ...
                  Conviction:
										<span className="text-[#243A57] font-medium">
											{isAye
												? votingData?.yes?.votes[0]?.lockPeriod
												: votingData?.no?.votes[0]?.lockPeriod}
                    x
										</span>
									</div>
								</div>
							) : (
								<div className="text-[#485F7D] font-medium text-xs ml-1 flex items-center">
                Abstain
								</div>
							)}
						</div>
						...
						votingData && (
							<div className="border-solid py-2 px-6 flex gap-2 border-[1px] rounded-b-[5px] bg-[#fff7ef] border-[#F89118]">
								...
								<div className="text-xs text-[#485F7D] flex items-center justify-center">
