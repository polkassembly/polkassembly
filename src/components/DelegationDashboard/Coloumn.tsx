...
const GetColumns = (status :ETrackDelegationStatus) => {
	...
	const AllColumns: ColumnsType<ITrackDataType> = [
		{ dataIndex: 'index', key: 1,
			render: (index) =>
			{
				return <div className='text-[14px] text-blue-light-high dark:text-blue-dark-high tracking-wide flex items-center justify-center font-normal'>{index}</div>;
			}, title: '#',width: '7%' },

		{ dataIndex: 'track', key: 2,
			render: (track) => {
				return <div className='text-[14px] text-blue-light-high dark:text-blue-dark-high tracking-wide flex items-center justify-start font-normal gap-1 max-lg:flex-col max-lg:gap-[2px] max-lg:text-center shrink-0'>
					{ handleTracksIcon(track, 24)}<span className=' flex items-center'>{track}</span>
		...
		{ return <div className='text-blue-light-high dark:text-blue-dark-high tracking-wider flex items-center justify-start font-medium gap-2 max-md:flex-col'>
			...
			{status.map((item: ETrackDelegationStatus, index:number)  => <h2 key={index}
				className={`text-[12px] ${item === ETrackDelegationStatus.Received_Delegation && 'bg-[#E7DCFF] dark:bg-[#6C2CF8]'} ${item === ETrackDelegationStatus.Delegated && 'bg-[#FFFBD8] dark:bg-[#695F0B]'} ${item === ETrackDelegationStatus.Undelegated && 'bg-[#FFDAD8] dark:bg-[#EF6158]'} rounded-[26px] py-[6px] px-[12px] text-center ${item === ETrackDelegationStatus.Received_Delegation && status.length > 1 && 'truncate w-[95px]'} `}>
	...
	const DelegatedColumns: ColumnsType<ITrackDataType> = [
		...
		{ dataIndex: 'track', key: 2,
			render: (track) => {
				return <div className='text-[14px] text-blue-light-high dark:text-blue-dark-high tracking-wide flex items-center justify-start font-normal gap-1 max-lg:flex-col max-lg:gap-[2px] max-lg:text-center'>
	...
	if(status === ETrackDelegationStatus.Delegated){
		const TrackColumn: ColumnsType<ITrackRowData> = [
			...
			{ dataIndex:'action', key:1, render: (action) => <div className='flex justify-center items-start'>
				<Button onClick={() => setOpen(true)} className='text-sm text-blue-light-high dark:text-blue-dark-high font-normal border-[1px] border-solid border-pink_primary h-[40px] flex items-center justify-center px-2 gap-2 max-md:h-auto max-md:gap-0 max-md:flex-col max-md:border-none max-md:p-2 max-md:shadow-none'>
