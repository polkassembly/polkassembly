...
const DashboardTrackListing = ({ className, posts, trackDetails }: Props) => {
	...
	const [openUndelegateModal, setOpenUndelegateModal] =
	...
	const getData = async () => {
		...
		if (data) {
			const rowData: ITrackRowData[] = data[0]?.delegations?.map(
	...
	return (
		<div className={`${className}`}>
          ...
          Dashboard
				...
				<span
					className="text-pink_primary text-sm cursor-pointer"
              ...
              status.map((item: ETrackDelegationStatus, index: number) => (
              	<span
                    ...
                    'bg-[#E7DCFF]'
              			...
              			item === ETrackDelegationStatus.Delegated && 'bg-[#FFFBD8]'
              		} ${
              			item === ETrackDelegationStatus.Undelegated &&
                    'bg-[#FFDAD8]'
              ...
              status.map((item: ETrackDelegationStatus, index: number) => (
              	<div className="flex gap-2 flex-col" key={index}>
					...
					{status.includes(ETrackDelegationStatus.Undelegated) && (
						<div className="bg-white flex pt-[24px] items-center flex-col text-[169px] pb-[33px] rounded-b-[14px]">
                  ...
                  Voting power for this track has not been delegated yet
									<Button
										...
										className="text-pink_primary font-normal tracking-wide text-sm ml-1 flex items-center justify-center max-md:mt-[10px] border-none shadow-none"
									>
			...
			{status.length > 0 ? (
				<div>
					<ActiveProposals
						...
						delegatedTo={
							status.includes(ETrackDelegationStatus.Delegated)
								? rowData.filter((row) => row.delegatedTo !== address)[0]
									.delegatedTo
								: null
						}
					/>
			...
			{rowData.filter((row) => row.delegatedTo !== address).length > 0 && (
				<UndelegateModal
					balance={
						new BN(
							rowData.filter((row) => row.delegatedTo !== address)[0]?.balance
						)
					}
					open={openUndelegateModal}
					setOpen={setOpenUndelegateModal}
					defaultTarget={
						rowData.filter((row) => row.delegatedTo !== address)[0]?.delegatedTo
					}
					trackNum={trackDetails?.trackId}
					conviction={
						rowData.filter((row) => row.delegatedTo !== address)[0]?.lockPeriod
					}
				/>
			)}
			<DelegateModal
