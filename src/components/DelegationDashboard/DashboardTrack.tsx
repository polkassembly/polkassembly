...
const DashboardTrackListing = ({ className, posts, trackDetails }: Props) => {
	...
	useEffect(() => {
		...
		const address =
			localStorage.getItem('delegationDashboardAddress') || '';
	...
	useEffect(() => {
		if (status.includes(ETrackDelegationStatus.Delegated)) {
			...
			status.includes(ETrackDelegationStatus.Received_Delegation)
		) {
			setShowTable(true);
	...
	return (
		<div className={`${className}`}>
						...
						<div className="bg-white flex pt-[24px] items-center flex-col text-[169px] pb-[33px] rounded-b-[14px]">
							...
							<div className="text-bodyBlue mt-[18px] text-center">
								<div className="text-sm tracking-[0.01em] font-normal mt-1 flex justify-center items-center max-md:flex-col">
									Voting power for this track has not been
									delegated yet
									<Button
										onClick={() =>
			...
			{rowData.filter((row) => row.delegatedTo !== address).length >
				0 && (
				<UndelegateModal
					balance={
						new BN(
