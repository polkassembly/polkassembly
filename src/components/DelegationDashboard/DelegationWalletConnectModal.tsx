		...
		>
			<Spin spinning={loading} indicator={<LoadingOutlined />}>
				<div className="flex flex-col">
					<h3 className="text-sm font-normal text-[#485F7D] text-center">
            Select a wallet
					</h3>
					<div className="flex items-center justify-center gap-x-4 mb-6">
						{defaultWallets[Wallet.POLKADOT] && (
							<WalletButton
								className={`${
									wallet === Wallet.POLKADOT
										? 'border border-solid border-pink_primary h-[44px] w-[56px]'
										: 'h-[44px] w-[56px]'
								}`}
								disabled={!apiReady}
								onClick={(event) =>
								...
								name="Polkadot"
								icon={
									<WalletIcon which={Wallet.POLKADOT} className="h-6 w-6" />
								}
							/>
						)}
						{defaultWallets[Wallet.TALISMAN] && (
							<WalletButton
								className={`${
						...
						{defaultWallets[Wallet.SUBWALLET] && (
							<WalletButton
								className={`${
								...
								name="Subwallet"
								icon={
									<WalletIcon which={Wallet.SUBWALLET} className="h-6 w-6" />
								}
							/>
						)}
						{(window as any).walletExtension?.isNovaWallet &&
              defaultWallets[Wallet.NOVAWALLET] && (
							<WalletButton
								disabled={!apiReady}
								className={`${
								...
								name="Nova Wallet"
								icon={
									<WalletIcon which={Wallet.NOVAWALLET} className="h-6 w-6" />
								}
							/>
						)}
						{['polymesh'].includes(network) &&
            defaultWallets[Wallet.POLYWALLET] ? (
								<WalletButton
									disabled={!apiReady}
									className={`${
										wallet === Wallet.POLYWALLET
									...
									name="PolyWallet"
									icon={
										<WalletIcon which={Wallet.POLYWALLET} className="h-6 w-6" />
									}
								/>
							) : null}
					</div>

					{Object.keys(defaultWallets || {}).length !== 0 &&
            accounts.length === 0 &&
            wallet &&
            wallet?.length !== 0 &&
            !loading && (
						<Alert
							message="For using delegation dashboard:"
							description={
					...
					{Object.keys(defaultWallets || {}).length === 0 && !loading && (
						<Alert
					...
					{!extensionOpen && (
						<Form form={form} disabled={loading}>
							{accounts.length > 0 ? (
								...
								/>
							) : !wallet && Object.keys(defaultWallets || {}).length !== 0 ? (
								<Alert type="info" showIcon message="Please select a wallet." />
							) : null}
						</Form>
					)}
				</div>
