...
export const formatedBalance = (balance: string, unit: string) => {
	...
	if (Number(formated?.[0]) > 0) {
		return formated?.[1]
			? `${formated[0]}.${formated[1].slice(0, 2)}`
			: '0';
	} else {
...
const ProfileBalances = ({ className, address }: Props) => {
	...
	return (
		<div
			className={
				'flex justify-between items-center w-full pl-[70px] max-md:pl-4 '
			}
		>
			<div
				className={`${className} flex py-[17px] items-center  h-full gap-1 max-md:px-[10px]`}
			>
				<div className="h-[71px] flex flex-col justify-start py-2 gap-1 ">
					<div className="text-[24px] font-semibold text-white tracking-[0.0015em] gap-1">
						...
						<span className="text-sm font-medium text-white tracking-[0.015em] ml-1">
							{unit}
						</span>
					</div>
				...
				<div className="flex gap-4 py-2 justify-start max-md:gap-2">
					<div className="h-[71px] flex flex-col py-2 gap-1">
						<div className="text-[24px] font-semibold text-white tracking-[0.0015em] gap-1">
							...
							<span className="text-sm font-medium text-white tracking-[0.015em] ml-[1px]">
								{unit}
							</span>
						</div>
