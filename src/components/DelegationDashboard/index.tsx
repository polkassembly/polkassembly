...
const DelegationDashboardHome = ({ className }: Props) => {
    ...
    return (
        <div className={`${className}`}>
            ...
            <h2 className=" text-[#243A57] mb-6 md:mb-5 mt-5 text-[24px] font-semibold max-lg:pt-[60px]">
                Delegation dashboard
            </h2>
            <DelegationProfile
                address={userDetails?.delegationDashboardAddress}
                username={userDetails?.username || ''}
                className="py-[24px] px-[34px]"
            />
            <div>
                {userDetails?.delegationDashboardAddress.length > 0 ? (
                    <DashboardTrackListing
                        className="mt-8 bg-white shadow-[0px 4px 6px rgba(0, 0, 0, 0.08)] rounded-[14px]"
