...
const DelegationDashboardHome = ({ className } : Props) => {
	...
	return <div className= { `${ className } delegation-dashboard` }>
		...
		<h2 className=' text-bodyBlue mb-6 md:mb-5 mt-5 text-[24px] font-semibold max-lg:pt-[60px]'>Delegation dashboard</h2>
		<DelegationProfile address={userDetails?.delegationDashboardAddress} username={userDetails?.username || ''} className='py-[24px] px-[34px]'/>
		<div >
			{userDetails?.delegationDashboardAddress ? <DashboardTrackListing className='mt-8 bg-white shadow-[0px 4px 6px rgba(0, 0, 0, 0.08)] rounded-[14px]' address={String(userDetails.delegationDashboardAddress)}/> : <Skeleton/>}
		</div>
