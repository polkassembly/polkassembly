...
const GovernanceCard: FC<IGovernanceProps> = (props) => {
	...
	const mainTitle = (
		<span className={tipReason ? 'tipTitle' : ''}>
			<div>{titleString}</div>
		</span>
	);
	...
	return (
		<>
			<div
				className={`${className} ${
					ownProposal && 'border-l-pink_primary border-l-4'
				} border-2 border-[#DCDFE350] border-solid hover:border-pink_primary hover:shadow-xl transition-all duration-200 sm:p-3 min-h-[120px] sm:flex xs:hidden`}
			>
				<div className="sm:flex flex-col sm:justify-between flex-1 sm:mt-2.5">
					<div className="flex justify-between items-center">
						<div className="flex flex-grow">
							<span className="font-medium text-center flex-none sm:w-[120px] text-bodyBlue">
					...
					<div className="flex justify-between items-center">
						<div className="flex flex-grow ml-[120px]">
							<h1 className="text-bodyBlue text-sm mt-0.5 flex overflow-hidden lg:max-w-none">
								<span className="break-all text-bodyBlue font-medium text-sm">
									{mainTitle}
								</span>
							</h1>
							<h2 className="text-bodyBlue font-medium text-sm">
								{subTitle}
							</h2>
						</div>
						{requestedAmount && (
							<div className="flex justify-center items-center">
								{requestedAmount > 100 ? (
									<span className="text-lightBlue text-sm font-medium sm:mr-[2.63rem]">
										{requestedAmountFormatted}{' '}
										{chainProperties[network]?.tokenSymbol}
									</span>
								) : (
									<span className="text-lightBlue text-sm font-medium sm:mr-[2.65rem]">
										{requestedAmountFormatted}{' '}
										{chainProperties[network]?.tokenSymbol}
									</span>
								)}
						...
						<div className="flex items-center gap-x-2">
							<div className="xs:hidden sm:flex items-center justify-center gap-x-1.5">
								...
								<span className="text-lightBlue">
									{getFormattedLike(postReactionCount['👍'])}
								</span>
							</div>
							...
							{tags && tags.length > 0 && (
								<>
									{tags?.slice(0, 2).map((tag, index) => (
										<div
											key={index}
											className="text-lightBlue rounded-xl px-[14px] py-[4px] border-[#D2D8E0] border-solid border-[1px] font-medium text-[10px]"
										>
											{tag}
										</div>
									))}
						...
						{tags && tags.length > 0 && (
							<>
								{tags?.map((tag, index) => (
									<div
										key={index}
										className="rounded-xl border-solid border-[1px] border-[#D2D8E0] px-[16px] py-[2px] font-normal text-[10px] text-lightBlue"
									>
										{tag}
									</div>
								))}
			...
			<div
				className={`${className} ${
					ownProposal && 'border-l-pink_primary border-l-4'
				} border-2 border-grey_light border-solid hover:border-pink_primary hover:shadow-xl transition-all duration-200  xs:px-2 xs:py-2 md:pb-6 min-h-[147px] xs:flex h-auto sm:hidden`}
			>
				<div className="sm:hidden xs:flex flex-col flex-1 xs:mt-1">
					{requestedAmount && (
						<div className="xs:flex xs:justify-end xs:mr-5">
							{requestedAmount > 100 ? (
								<span className="text-lightBlue text-sm font-medium sm:mr-[54px]">
									{requestedAmountFormatted}{' '}
									{chainProperties[network]?.tokenSymbol}
								</span>
							) : (
								<span className="text-lightBlue text-sm font-medium sm:mr-20">
									{requestedAmount}{' '}
									{chainProperties[network]?.tokenSymbol}
								</span>
							)}
					...
					<div className="sm:hidden xs:flex xs:justify-start gap-x-2 lg:items-start lg:flex-row my-2 ">
						<span className="font-medium text-center xs:w-[45px] text-bodyBlue xs:mt-0">
							#{isTip ? tip_index : onchainId}
						</span>
						<div className="xs:mt-0 lg:mt-0">
							<h1 className="text-bodyBlue flex overflow-hidden lg:max-w-none">
								<span className="text-bodyBlue text-sm font-medium mt-0 xs:mt-[-1.5px]">
									{mainTitle}
								</span>
							</h1>
							<h2 className="text-bodyBlue font-medium text-sm">
						...
						<div className="sm:hidden xs:flex justify-between items-center gap-x-2">
							{spam_users_count &&
							typeof spam_users_count === 'number' &&
							spam_users_count > 0 ? (
					...
					<div className="mt-0 sm:gap-2.5 xs:gap-0 font-medium text-bodyBlue text-xs sm:hidden xs:flex flex-col lg:flex-row items-start lg:items-center">
						<div className="sm:hidden xs:flex xs:justify-start xs:flex-wrap">
							...
							<Divider
								type="vertical"
								className="max-lg:hidden xs:inline-block xs:mt-0.5"
								style={{ borderLeft: '1px solid #485F7D' }}
							/>
							{relativeCreatedAt && (
								<>
									<div className="flex text-lightBlue xs:-mt-0.5 mt-0 items-center">
										<ClockCircleOutlined className="mr-1 mt-0 xs:-mt-0.5" />{' '}
										{relativeCreatedAt}
									</div>
								</>
							)}
						...
						<div className="xs:flex justify-between items-center xs:mt-3 xs:gap-x-2">
							...
							{tags && tags.length > 0
								? status && (
										<StatusTag
											className="xs:mt-0 xs:absolute xs:right-[5%]"
											status={status}
										/>
								  )
								: null}
				...
				>
					<div className="flex">
						...
						<h2 className="text-lg tracking-wide font-semibold text-bodyBlue mb-2">
							Tags
						</h2>
					</div>
					<div className="w-full h-[1px] bg-[#D2D8E0]" />
