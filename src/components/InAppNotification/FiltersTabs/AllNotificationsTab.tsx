...
const AllNotificationsTab = ({ inPage, closePopover, isStopInterval, setStopInterval }: INotificationsTab) => {
	...
	const { popupNotifications, totalNotificationsCount, allNotifications, unreadNotificationsCount, popupActiveFilter } = useInAppNotificationsSelector();
	const [loading, setLoading] = useState(false);
	const [loadingTime, setLoadingTime] = useState(0);
	const [page, setPage] = useState(Number(router.query.page as string) || 1);
	const pageActiveFilter = (router.query.filter as ECustomNotificationFilters) || ECustomNotificationFilters.ALL;
	...
	const handleModifyData = (data: IInAppNotificationResponse) => {
		...
		dispatch(inAppNotificationsActions.updateAllNotifications(notifications));
	...
	const getNotifications = async (pageNum?: number, isLoading?: boolean) => {
		...
		const { data, error } = await nextApiClientFetch<IInAppNotificationResponse>('/api/v1/inAppNotifications/get-notifications', {
			filterBy: ECustomNotificationFilters.ALL,
			page: inPage ? pageNum || page || 1 : 1
		});

		if (data) {
			if (inPage) {
				if (data.filterBy !== pageActiveFilter || pageActiveFilter !== ECustomNotificationFilters.ALL) {
					setLoading(false);
					return;
				}
	...
	return (
		<Spin
			...
			className='h-[500px]'
		>
			{totalNotificationsCount > 0 && (inPage || !!popupNotifications?.all?.length) ? (
				inPage ? (
					<div>
