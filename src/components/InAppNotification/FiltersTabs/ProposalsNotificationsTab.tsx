...
const ProposalsNotificationsTab = ({ inPage, closePopover, isStopInterval, setStopInterval }: INotificationsTab) => {
	const dispatch = useDispatch();
	const router = useRouter();
	const { resolvedTheme: theme } = useTheme();
	const { id: userId, networkPreferences } = useUserDetailsSelector();
	const { popupNotifications, totalNotificationsCount, proposalsNotifications, unreadNotificationsCount, popupActiveFilter } = useInAppNotificationsSelector();
	const [loading, setLoading] = useState(false);
	const [loadingTime, setLoadingTime] = useState(0);
	const [page, setPage] = useState(Number(router.query.page as string) || 1);
	const pageActiveFilter = (router?.query?.filter as ECustomNotificationFilters) || ECustomNotificationFilters.PROPOSALS;
	...
	const handleModifyData = (data: IInAppNotificationResponse) => {
		...
		dispatch(inAppNotificationsActions.updateProposalsNotifications(notifications));
	...
	const getNotifications = async (pageNum?: number, isLoading?: boolean) => {
		...
		const { data, error } = await nextApiClientFetch<IInAppNotificationResponse>('/api/v1/inAppNotifications/get-notifications', {
			filterBy: ECustomNotificationFilters.PROPOSALS,
			page: inPage ? pageNum || page || 1 : 1
		});

		if (data) {
			if (inPage) {
				if (data.filterBy !== pageActiveFilter || pageActiveFilter !== ECustomNotificationFilters.PROPOSALS) {
					setLoading(false);
					return;
				}
	...
	return (
		<Spin
			...
			className='h-[500px]'
		>
			{totalNotificationsCount > 0 && (inPage || !!popupNotifications?.proposals?.length) ? (
				inPage ? (
					<div>
						<NotificationsContainer
							count={totalNotificationsCount}
							data={proposalsNotifications || []}
							inPage={inPage}
					...
					<div className={classNames('flex flex-col gap-2 ', inPage ? '' : 'h-[450px] overflow-y-auto')}>
						<NotificationsContainer
							...
							data={popupNotifications?.proposals || []}
							inPage={inPage}
							title='Unread'
...
export default ProposalsNotificationsTab;
