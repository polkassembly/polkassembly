...
const TrackListingCard: FC<ITrackListingCardProps> = (props) => {
  const { posts, className, setTrackName, fellowshipReferendumPostOrigins } =
    props;
  const items = [
    {
      children: (
  ...
  const defaultActiveTab =
    trackName &&
    ['All', ...fellowshipReferendumPostOrigins].includes(String(trackName))
      ? String(trackName)
      : 'All';
  ...
  return (
    ...
    >
      ...
      />
      {(posts?.[activeTab]?.data?.count || 0) > 0 &&
        (posts as any)[activeTab].data.count > LISTING_LIMIT && (
          <Pagination
