...
const DiscussionCard: FC<IDiscussionProps> = (props) => {
  ...
  return (
    <>
      <div
        className={`${
          ownPost && 'border-l-pink_primary border-l-4'
        } border-2 border-solid border-[#DCDFE350] hover:border-pink_primary hover:shadow-xl transition-all duration-200 p-3 md:p-4 min-h-[120px] sm:flex xs:hidden ${className}`}
      >
        <span className="font-medium text-center flex-none sm:w-[120px] text-bodyBlue sm:mt-2">
          #{post_id}
        </span>
                ...
                <>
                  {tags?.slice(0, 2).map((tag, index) => (
                    <div
                      key={index}
                      style={{ fontSize: '10px' }}
                      className="text-lightBlue rounded-xl px-[14px] py-[4px] border-[#D2D8E0] border-solid border-[1px] font-medium"
                    >
                      {tag}
                    </div>
                  ))}
      ...
      <div
        className={`${
          ownPost && 'border-l-pink_primary border-l-4'
        } border-2 border-solid border-grey_light hover:border-pink_primary hover:shadow-xl transition-all duration-200 xs:p-2 md:p-4 min-h-[150px] h-auto xs:flex sm:hidden ${className}`}
      >
        <div className="sm:hidden xs:flex flex-col flex-1 xs:mt-1">
          <div className="sm:hidden xs:flex xs:justify-start gap-x-2 lg:items-start lg:flex-row my-2">
            <span className="font-medium text-center mr-2 flex-none xs:w-[45px] text-bodyBlue xs:mt-0">
              #{post_id}
            </span>
            <div className="xs:mt-0 lg:mt-0">
              <h1 className="text-bodyBlue font-medium flex">{title}</h1>
            </div>
            <div className="flex justify-between items-center">
              {spam_users_count &&
              typeof spam_users_count === 'number' &&
              ...
              {relativeCreatedAt && (
                <>
                  <div className="xs:flex xs:text-lightBlue xs:-mt-0.5 mt-0 lg:flex items-center text-sm">
                    <ClockCircleOutlined className="mr-1 mt-0" />{' '}
                    {relativeCreatedAt}
                  </div>
                </>
              )}
