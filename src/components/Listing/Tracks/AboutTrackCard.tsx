...
export const getTrackData = (
	...
	if (tracks) {
		...
		if (tracksArr && Array.isArray(tracksArr) && tracksArr.length > 0) {
			const currTrackMetaDataArr = tracksArr.find(
				(v) =>
					v &&
          Array.isArray(v) &&
          v.length > 1 &&
        ...
        Array.isArray(currTrackMetaDataArr) &&
        currTrackMetaDataArr.length >= 2
			) {
			...
			<div className="mt-8 text-xs w-full flex flex-wrap lg:gap-x-24">
				<div className="w-full sm:w-1/2 md:w-1/3 lg:w-auto">
					<div className="flex flex-col">
						<div className="font-medium text-sm text-lightBlue whitespace-pre">
              Max Deciding
						</div>
              ...
              Confirm Period
				...
				<div className="w-full sm:w-1/2 md:w-1/3 lg:w-auto">
					<div className="flex flex-col">
						<div className="font-medium leading-5 text-sm text-lightBlue whitespace-pre">
              ...
              Decision Deposit
                ...
                formatUSDWithUnits(
                	formatBnBalance(
                		`${trackMetaData.decisionDeposit}`.startsWith('0x')
                			? new BN(
                				`${trackMetaData.decisionDeposit}`.slice(2),
                				'hex'
                			)
                			: trackMetaData.decisionDeposit,
                		{
