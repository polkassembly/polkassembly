	...
	return (
		<Modal
			...
			className={`${poppins.variable} ${poppins.className} ${
				isDelegate ? 'delegate' : 'undelegate'
			}`}
		...
		>
			<div className="flex justify-center items-center flex-col -mt-[132px]">
				...
				<h2 className="text-[20px] font-semibold tracking-[0.0015em] mt-4">
					{isDelegate ? `${title} successfully` : 'Undelegated successfully'}
				</h2>
				{isDelegate && (
					<div className="flex flex-col justify-center items-center gap-[14px]">
						...
						{vote === EVoteDecisionType.SPLIT && (
							<div className=" flex flex-wrap justify-center font-normal text-sm text-[#243A57]">
								{' '}
								<span className="mr-3">
									<span className="font-semibold"> Aye: </span>
									<span className="font-normal">
										{ayeVoteValue
											? formatedBalance(ayeVoteValue.toString(), unit)
											: 0}
										{` ${unit}`}
									</span>
								</span>{' '}
								<span className="mr-3">
									...
									<span className="font-normal">
										{nayVoteValue
											? formatedBalance(nayVoteValue.toString(), unit)
											: 0}
										{` ${unit}`}
									</span>
								</span>
						...
						{vote === EVoteDecisionType.ABSTAIN && (
							<div className="flex flex-wrap justify-center font-normal text-sm text-[#243A57]">
								...
								<span className="mr-3">
									...
									<span className="font-normal">
										{abstainVoteValue
											? formatedBalance(abstainVoteValue.toString(), unit)
											: 0}
										{` ${unit}`}
									</span>
								</span>{' '}
                  ...
                  Vote :
									{vote === EVoteDecisionType.AYE ? (
										...
										<div>
											...
											<span className="mb-[5px] capitalize font-medium text-[#243A57]">
												{vote}
											</span>
										</div>
									) : vote === EVoteDecisionType.SPLIT ? (
                ...
                Conviction:
							...
							{tracks && (
								<div className="flex gap-[35px] text-sm text-[#485F7D]">
                  Track(s):
									<span>
										<div
											className={`flex flex-col gap-1 min-h-[50px] max-h-[100px] text-[#243A57] pr-2 font-medium ${
										...
										>
											{tracks.map((track, index) => (
												<div key={index}>
													{track} #
													{networkTrackInfo[network][track.toString()].trackId}
												</div>
											))}
										</div>
									</span>
                  ...
                  Time of Vote :{' '}
									<span className="font-medium text-[#243A57]">{votedAt}</span>
								</div>
							)}
						</div>
					</div>
