...
const OptionPoll: FC<IOptionPollProps> = ({
  className,
  optionPollId,
  question,
  options,
  ...
  const castVote = useCallback(
    async (option: string) => {
      ...
      setLoading(true);
      const { error: deleteVoteErr, data: votesAfterDelete } =
                  ...
                  if (optionPoll.id === optionPollId) {
                    optionPoll.option_poll_votes =
                      optionPoll?.option_poll_votes?.filter(
                        (optionPollVote) => optionPollVote.user_id !== id,
                      ) || [];
                  }
                  return {
                    ...optionPoll,
      ...
      const { error: addVoteErr, data: votesAfterAdd } =
        await nextApiClientFetch<MessageType>(
    ...
    <GovSidebarCard className={className}>
      <div className="flex items-center mb-6">
        <h3 className="dashboard-heading mb-0">
          <span className="text-navBlue mr-1">Poll:</span>
          {question}?
        </h3>
        <HelperTooltip
          className="ml-2"
        ...
        {options.map((option) => (
          ...
          >
            ...
            <Progress
              ...
              percent={
                totalVotes &&
                Math.round(((optionMap[option] || 0) * 100) / totalVotes)
              }
