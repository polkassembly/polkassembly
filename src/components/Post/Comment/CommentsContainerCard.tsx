...
const CommentsContainerCard: FC<ICommentsContainerProps> = (props) => {
	...
	return (
		<div className={`${className} mb-6 w-full rounded-xxl bg-white p-3 drop-shadow-md dark:bg-section-dark-overlay md:p-4 lg:p-6 `}>
			{id ? (
				<>
					{isGrantClosed ? (
						<Alert
			...
			{Boolean(allComments?.length) && timelines.length >= 1 && !loading && (
				...
				>
					<span className={`${poppins.variable} ${poppins.className} text-xl font-semibold text-blue-light-high dark:text-blue-dark-high`}>
						Comments
						<span className='ml-[2px] text-base font-normal'>({allComments.length || 0})</span>
					</span>
...
// @ts-ignore
export default React.memo(styled(CommentsContainerCard)`
	.ant-anchor-wrapper {
		.ant-anchor {
			display: flex;
