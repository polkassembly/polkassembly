...
const editReplyKey = (replyId: string) =>
	`reply:${replyId}:${global.window.location.href}`;
const newReplyKey = (commentId: string) =>
...
const EditableReplyContent = ({
	...
	useEffect(() => {
		const localContent =
      global.window.localStorage.getItem(editReplyKey(replyId)) || '';
	...
	const handleSave = async () => {
		...
		setLoading(true);
		const { data, error: editReplyError } =
      await nextApiClientFetch<MessageType>(
      	'api/v1/auth/actions/editCommentReply',
      	{
      		commentId,
      		content: newContent,
      		postId: postIndex,
	...
	const deleteReply = async () => {
		setLoading(true);
		const { data, error: deleteReplyError } =
      await nextApiClientFetch<MessageType>(
      	'api/v1/auth/actions/deleteCommentReply',
      	{
      		commentId,
      		postId: postIndex,
      		postType: postType,
		...
		<>
			<div className={className}>
				...
				{isEditing ? (
					...
					<>
						<Markdown
							className="py-2 px-2 md:px-4 bg-blue-grey rounded-b-md text-sm"
							md={content}
						/>
						<div className="flex items-center flex-wrap">
							{id === userId && (
								<Button
									className={
										'text-pink_primary flex items-center border-none shadow-none'
									}
									disabled={loading}
									onClick={toggleEdit}
								>
							...
							{id === userId && (
								<Button
									className={
										'text-pink_primary flex items-center border-none shadow-none text-xs'
									}
									onClick={deleteReply}
								>
									<DeleteOutlined />
							...
							{id ? (
								reply.reply_source === 'subsquare' ? (
									...
									>
										<Button
											disabled={true}
											className="text-pink_primary flex items-center border-none shadow-none text-xs disabled-reply"
										>
