...
const CreatePost = ({ className, proposalType }: Props) => {
    ...
    const createPoll = async (postId: number) => {
        ...
        const { error: apiError } = await nextApiClientFetch(
        ...
        try {
            ...
            const { data, error: apiError } =
                await nextApiClientFetch<CreatePostResponseType>(
                    'api/v1/auth/actions/createPost',
                    {
                        content,
                        gov_type: govType,
                        proposalType,
    ...
    return (
        <div className={className}>
            ...
            <div className="flex flex-col mt-6 bg-white p-4 md:p-8 rounded-md w-full shadow-md mb-4">
                ...
                >
                    <label className="text-sidebarBlue font-normal text-sm mb-1 tracking-wide">
                    ...
                    <div className="flex items-center">
                        ...
                        />
                        <span className="ml-2 text-sidebarBlue text-sm">
                            Add poll to{' '}
                            {proposalType === ProposalType.DISCUSSIONS
                                ? 'discussion'
                                : 'grant'}
                    ...
                    <h5 className="text-sm text-color mt-8 font-normal">
                        Select Governance version{' '}
                        <span className="text-red-500">*</span>
                    </h5>
                    <Radio.Group
                        className="font-normal text-xs p-1"
                        onChange={(e) => setGovType(e.target.value)}
                        value={govType}
                    >
                        <Radio
                            className={`font-normal text-xs text-navBlue ${
                                govType === 'gov_1' && 'text-pink_primary'
                            }`}
                            value="gov_1"
                            defaultChecked
                        >
                        ...
                        <Radio
                            className={`font-normal text-xs text-navBlue ${
                                govType === 'open_gov' && 'text-pink_primary'
                            }`}
                            value="open_gov"
                    ...
                    <Form.Item>
                        <Button
                            htmlType="submit"
                            disabled={
                                !currentUser.id || formDisabled || loading
                            }
                            className="mt-10 bg-pink_primary text-white border-white hover:bg-pink_secondary flex items-center justify-center rounded-md text-lg h-[50px] w-[215px]"
                        >
                            Create Post
                        </Button>
                    </Form.Item>
