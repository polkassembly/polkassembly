...
const VoterRow: FC<IVoterRow> = ({ currentKey, setActiveKey, voteType, voteData, className, setDelegationVoteModal, index, tally, referendumId, decision, isReferendum2 }) => {
	...
	return voteData?.delegatedVotes?.length > 0 && voteData?.decision !== 'abstain' ? (
		<StyledCollapse
			theme={theme}
			className={`${active ? 'border-t-2 border-pink_primary' : 'border-t-[1px] border-[#D2D8E0] dark:border-separatorDark'} w-[550px] gap-[0px] rounded-none border-0 ${className}`}
			size='large'
		...
		>
			...
			>
				<div className='flex flex-col gap-4'>
					<div className='flex items-center gap-[60px] border-x-0 border-y-2 border-dashed border-[#D2D8E0] py-4 dark:border-separatorDark'>
					...
					<div>
						...
						<div className='flex justify-between'>
							<div className='flex w-[200px] flex-col gap-1'>
								...
								<div className='flex justify-between'>
									<span className='flex items-center gap-1 text-xs text-[#576D8B] dark:text-blue-dark-medium'>
								...
								<div className='flex justify-between'>
									...
									<span className='text-xs text-bodyBlue dark:text-white'>{voteData.lockPeriod ? `${voteData.lockPeriod}x${voteData?.delegatedVotes?.length > 0 ? '/d' : ''}` : '0.1x'}</span>
								</div>
								<div className='flex justify-between'>
									<span className='flex items-center gap-1 text-xs text-[#576D8B] dark:text-blue-dark-medium'>
										<CapitalIcon /> Capital
							...
							<div className='border-y-0 border-l-2 border-r-0 border-dashed border-[#D2D8E0] dark:border-separatorDark'></div>
					...
					<Divider
						dashed
						className='m-0 mt-2 border-[2px] border-x-0 border-b-0 border-[#D2D8E0] dark:border-separatorDark'
					/>
					<div>
						<p className='mb-4 text-sm font-medium text-bodyBlue dark:text-white'>Delegation list</p>
						<div className='mb-2 flex items-center text-xs font-semibold dark:text-white'>
							<div className='w-[200px] text-lightBlue dark:text-icon-dark-inactive'>Delegators</div>
							<div className='w-[110px] items-center text-lightBlue dark:text-icon-dark-inactive'>Amount</div>
							{network !== AllNetworks.COLLECTIVES ? <div className='ml-1 w-[110px] items-center text-lightBlue dark:text-icon-dark-inactive'>Conviction</div> : null}
							<div className='w-[100px] items-center text-lightBlue dark:text-icon-dark-inactive'>Voting Power</div>
						</div>
		...
		<div className={`w-[552px] border-x-0 border-y-0 border-t border-solid border-[#D2D8E0] dark:bg-[#222] dark:border-separatorDark px-[10px] py-4 text-sm text-bodyBlue ${className}`}>
			<Title />
		</div>
	);
};
