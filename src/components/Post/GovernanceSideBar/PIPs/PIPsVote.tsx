...
const PIPsVote = ({ className, referendumId, onAccountChange, lastVote, setLastVote, proposalType, address, hash }: Props) => {
	...
	const handleSubmit = async () => {
		...
		await executeTx({ address, api, errorMessageFallback: 'Transaction failed.', network, onFailed, onSuccess, tx: voteTx });
	};
		...
		<>
			<div className={className}>
				...
				<Modal
					...
					onCancel={() => setShowModal(false)}
					footer={false}
					className={`w-[500px] ${poppins.variable} ${poppins.className} alignment-close vote-referendum max-h-[605px] rounded-[6px] max-md:w-full `}
