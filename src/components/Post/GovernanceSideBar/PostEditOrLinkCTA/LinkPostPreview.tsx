...
const LinkPostPreview: FC<{ post?: ILinkPostStartResponse; className?: string }> = (props) => {
	...
	const { tags } = post;
	return (
		<section className={`border border-solid border-[rgba(72,95,125,0.2)] rounded-[4px] p-4 ${className}`}>
			<h3
			...
			{tags && Array.isArray(tags) && tags.length > 0 &&<div className='flex my-3 gap-[8px]'>
				{
					tags?.map((tag,index ) => {
						return (
							<div
