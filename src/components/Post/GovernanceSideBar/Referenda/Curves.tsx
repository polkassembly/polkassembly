...
const Curves: FC<ICurvesProps> = (props) => {
	...
	return (
		<Spin indicator={<LoadingOutlined />} spinning={curvesLoading}>
			{curvesError ? (
				<p className="text-red-500 font-medium text-center">{curvesError}</p>
			) : (
				<section>
					<article className="-mx-3 md:m-0">
						<Chart.Line
									...
									tooltip: {
										callbacks: {
											label(tooltipItem: any) {
												...
												if (
													['Approval', 'Current Approval'].includes(
														dataset.label
													)
												) {
													return '';
                            ...
                            data.datasets[2].data[dataIndex];
													const currentSupport =
                            data.datasets[3].data[dataIndex];
												...
												const result = `Time: ${(hs / 60).toFixed(
													0
								...
								scales: {
									x: {
										...
										ticks: {
											...
											max: labelsLength,
											stepSize: Math.round(
												labelsLength / (labelsLength / (60 * 24))
											)
										} as any,
										title: {
											display: true,
