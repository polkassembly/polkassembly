...
const MultisigVoteInitiatedModal = ({ className, open, setOpen, address, multisig, balance, conviction, title = 'Delegated', vote, votedAt, ayeVoteValue, nayVoteValue, abstainVoteValue }: Props) => {
	...
	>
		<div className='flex justify-center items-center flex-col -mt-[132px]'>
			<SuccessIcon />
			<h2 className='text-[20px] font-semibold tracking-[0.0015em] mt-4'>{title}</h2>
			<div className='flex flex-col justify-center items-center gap-[14px]'>
				<div className='text-pink_primary text-[24px] font-semibold'>{formatedBalance(balance.toString(), unit)}{` ${unit}`}</div>
				{
				...
				<div className='flex-col flex items-start justify-center gap-[10px]'>
					...
					<div className='flex gap-3 text-sm text-[#485F7D] font-normal'>With address: <span className='font-medium'><Address address={address} className='address' displayInline={true} /> </span>
					</div>

					<div className='flex gap-3 text-sm text-[#485F7D] font-normal'>With Multisig: <span className='font-medium'><Address address={multisig} className='address' displayInline={true} /> </span>
					</div>
