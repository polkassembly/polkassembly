...
const ReferendaV2Messages: FC<IReferendaV2Messages> = (props) => {
    ...
    const Button: FC<IButtonProps> = (props) => {
        ...
        return (
            <button
                ...
                className="cursor-pointer flex items-center justify-center border-none outline-none bg-[#FEF2F8] w-[30px] h-[30px] rounded-full text-base font-normal leading-[24px] tracking-[0.01em] text-lightBlue"
            >
                {children}
            </button>
        );
    ...
    useEffect(() => {
        ...
        const decisionPeriodStartsAt =
            decidingStatusBlock && decidingStatusBlock.timestamp
                ? dayjs(decidingStatusBlock.timestamp)
                : prepare.periodEndsAt;
        const decision = getPeriodData(
            network,
        ...
        const confirmPeriodEndsAt =
            confirmStartedStatusBlock && confirmStartedStatusBlock.timestamp
                ? dayjs(confirmStartedStatusBlock.timestamp)
                : decision.periodEndsAt;
        const confirm = getPeriodData(
            network,
            confirmPeriodEndsAt,
            trackData,
            "confirmPeriod"
        ...
        const minEnactmentPeriodStartsAt =
            confirmedStatusBlock && confirmedStatusBlock.timestamp
                ? dayjs(confirmedStatusBlock.timestamp)
                : confirm.periodEndsAt;
        const minEnactment = getPeriodData(
            network,
        ...
        <>
            {!decidingStatusBlock && !isProposalFailed && (
                <GovSidebarCard>
                    <div className="flex items-center justify-between">
                        <h3 className="m-0 text-bodyBlue font-medium text-xl leading-6 tracking-[0.0015em]">
                            Prepare Period
                        </h3>
                        <Button>1</Button>
                    </div>
                    ...
                    <p className="p-0 m-0 flex items-center justify-between mt-3.5 leading-[22px]">
                        <>
                            <span className="text-bodyBlue text-sm font-normal">
                                Prepare Period
                            </span>
                ...
                !isProposalFailed && (
                    <GovSidebarCard>
                        ...
                        <div className="mt-[30px]">
                            <Progress
                                className="m-0 p-0 flex items-center rounded-lg"
            ...
            {isProposalPassed ? (
                <>
                    ...
                    minEnactment.periodCardVisible ? (
                        <GovSidebarCard>
                            <div className="flex items-center justify-between">
                                <h3 className="m-0 text-bodyBlue font-medium text-xl leading-6 tracking-[0.0015em]">
                                    Proposal Passed
            ...
            <Modal
                open={open}
                title={
                    <div className="flex items-center justify-between gap-x-5 py-3 px-2">
                        <h3 className="text-bodyBlue font-medium text-xl leading-[24px] tracking-[0.0015em] m-0 p-0">
                            Status
                        </h3>
                        <button
                            onClick={() => setOpen(false)}
                            className="border-none outline-none cursor-pointer bg-transparent flex items-center justify-center"
                        >
                            <CloseIcon />
                        </button>
                    </div>
                    ...
                    <article className="flex gap-x-[23px]">
                        ...
                        <div>
                            <div className="mt-1.5 relative">
                                <span
                                    className={`absolute -top-[2px] -left-[37px] w-[26px] h-[26px] font-medium text-base leading-0 flex items-center justify-center rounded-full ${
                                        prepare.periodPercent > 0
                                            ? "bg-pink_primary text-white"
                                            : "bg-[#FFD3EC] text-[#7A8BA1]"
                                    }`}
                                >
                                    1
                                </span>
                            ...
                            <h4 className="text-base leading-[24px] tracking-[0.01em] font-medium">
                                Prepare Period
                            </h4>
                            <p className="text-sm leading-[21px] tracking-[0.01em]">
                                The prepare period is used to avoid decision
                                sniping. It occurs before a referendum goes into
                                voting.
                            </p>
                        </div>
                    </article>
                    <article className="flex gap-x-[23px]">
                        ...
                        <div>
                            ...
                            <ul className="text-sm leading-[21px] tracking-[0.01em] px-5">
                                <li>
                                    A referendum will be in voting till the
                                    decision period is completed or the proposal
                                    is passed.
                                </li>
                                <li>
                                    For a referendum to pass, the support and
                                    approval should be greater than the
                                    threshold for the track for the confirmation
                                    period.
                                </li>
                                <li>
                                    If the referendum does not pass during the
                                    decision period, it is considered as failed.
                                </li>
                            </ul>
                        </div>
                    </article>
                    <article className="flex gap-x-[23px]">
                        ...
                        <div>
                            ...
                            <ul className="text-sm leading-[21px] tracking-[0.01em] m-0 p-0 px-5">
                                <li>
                                    A referendum is executed after the
                                    completion of the enactment period.
                                </li>
                                <li>
                                    For treasury referenda, the funds will be
                                    disbursed after completion of the funds
                                    disbursal period.
                                </li>
                            </ul>
                        </div>
...
const FailedReferendaText: FC<{
    ...
    const BlockElement = (
        <a
            className="text-pink_primary font-medium"
    ...
    const isSupportLess =
        Number(progress.support) < Number(progress.supportThreshold);
    const isApprovalLess =
        Number(progress.approval) < Number(progress.approvalThreshold);
    return (
        <>
            {status === "Cancelled" ? (
                <>
                    The proposal has been cancelled at block {BlockElement} via
                    the referendum canceller track
                </>
            ) : status === "Killed" ? (
                <>
                    The proposal has been killed at block {BlockElement} via the
                    referendum killer track and the deposit was slashed
                </>
            ) : status === "TimedOut" ? (
                <>
                    The proposal has been timed out as the decision deposit was
                    not placed in due time
                </>
            ) : (
                <>
                    {isSupportLess && isApprovalLess ? (
                        <>
                            <p>
                                Referendum failed because either of the 2
                                reasons:
                            </p>
                            <ul className="pl-5 m-0">
                                <li>
                                    The support was lesser than the threshold
                                    for this track.
                                </li>
                                <li>
                                    The approval was lesser than the threshold
                                    for this track.
                                </li>
                            </ul>
                        </>
                        ...
                        <>
                            The support was lesser than the threshold for this
                            track.
                        </>
                    ) : (
                        <>
                            The approval was lesser than the threshold for this
                            track.
                        </>
                    )}
                </>
