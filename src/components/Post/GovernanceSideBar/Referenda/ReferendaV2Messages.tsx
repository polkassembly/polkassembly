...
export const getPeriodData = (
	...
	if (period) {
		...
		periodPercent = Math.round(
			(dayjs().diff(date, 'minute') / periodEndsAt.diff(date, 'minute')) * 100
		);
	}
	...
	return (
		<>
			...
			>
					...
					<article className="flex gap-x-[23px]">
						<div className="w-[4.5px] h-[150px] bg-[#FCE5F2] rounded-full">
							<div
								style={{
									height: `${
										(Math.min(prepare.periodPercent, 100) / 100) * 150
									}px`
								}}
                ...
                The prepare period is used to avoid decision sniping. It occurs
                before a referendum goes into voting.
							</p>
						</div>
					</article>
					<article className="flex gap-x-[23px]">
						<div className="w-[4.5px] h-[300px] md:h-[250px] bg-[#FCE5F2] rounded-full overflow-hidden">
							<div
								style={{
									height: `${
										decidingStatusBlock
											? (Math.min(decision.periodPercent, 100) / 100) * 300
											: 0
									}px`
								}}
								className="bg-pink_primary rounded-full border-0"
						...
						<div>
							<div className="relative">
								<span
									className={`absolute -top-[2px] -left-[37px] w-[26px] h-[26px] font-medium text-base leading-0 flex items-center justify-center rounded-full ${
										decision.periodPercent > 0 && decidingStatusBlock
											? 'bg-pink_primary text-white'
											: 'bg-[#FFD3EC] text-[#7A8BA1]'
									}`}
								>
                ...
                Voting Period
							...
							<ul className="text-sm leading-[21px] tracking-[0.01em] px-5">
								<li>
                  A referendum will be in voting till the decision period is
                  completed or the proposal is passed.
								</li>
								<li>
                  For a referendum to pass, the support and approval should be
                  greater than the threshold for the track for the confirmation
                  period.
								</li>
								<li>
                  If the referendum does not pass during the decision period, it
                ...
                After Voting Period
							...
							<ul className="text-sm leading-[21px] tracking-[0.01em] m-0 p-0 px-5">
								<li>
                  A referendum is executed after the completion of the enactment
                  period.
								</li>
								<li>
                  For treasury referenda, the funds will be disbursed after
                  completion of the funds disbursal period.
								</li>
							</ul>
						</div>
					</article>
  ...
  progress: IProgress;
	...
	return (
		<>
			{status === 'Cancelled' ? (
				<>
          The proposal has been cancelled at block {BlockElement} via the
          referendum canceller track
				</>
			) : status === 'Killed' ? (
				<>
          ...
          referendum killer track and the deposit was slashed
				...
				<>
          The proposal has been timed out as the decision deposit was not placed
          in due time
				</>
			) : (
				<>
