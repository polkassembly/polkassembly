...
const ReferendumV2VoteInfo: FC<IReferendumV2VoteInfoProps> = ({
	className,
	tally,
	setOpen,
	setThresholdOpen,
	...
	useEffect(() => {
		...
		if (
			...
			setTallyData({
				ayes: String(tally?.ayes).startsWith('0x')
					? new BN(tally?.ayes || 0, 'hex')
					: new BN(tally?.ayes || 0),
				nays: String(tally?.nays).startsWith('0x')
					? new BN(tally?.nays || 0, 'hex')
					: new BN(tally?.nays || 0),
		...
		(async () => {
			const referendumInfoOf =
				await api.query.referenda.referendumInfoFor(postIndex);
			const parsedReferendumInfo: any = referendumInfoOf.toJSON();
			if (parsedReferendumInfo?.ongoing?.tally) {
				setTallyData({
					ayes:
						typeof parsedReferendumInfo.ongoing.tally.ayes ===
						'string'
							? new BN(
					...
					nays:
						typeof parsedReferendumInfo.ongoing.tally.nays ===
						'string'
							? new BN(
					...
					support:
						typeof parsedReferendumInfo.ongoing.tally.support ===
						'string'
							? new BN(
	...
	return (
		<GovSidebarCard className={className}>
			<div className="flex items-center justify-between relative z-50">
				<h6 className="text-bodyBlue font-medium text-xl leading-6 m-0 p-0">
					Voting
				</h6>
				<div className="flex items-center gap-x-2">
					...
					<button
						...
						className="border-none outline-none bg-transparent flex items-center cursor-pointer justify-center text-lg text-navBlue hover:text-pink_primary"
					>
						<VoteCalculationIcon />
					</button>
				</div>
			...
			<Spin spinning={isLoading} indicator={<LoadingOutlined />}>
				...
				<section className="flex items-center gap-x-4 border-0 border-t-[0.75px] border-solid border-[#D2D8E0] mt-[18px] pt-[18px] pb-[14px]">
					...
					<Modal
						...
						closeIcon={<CloseIcon />}
						title={
							<h2 className="text-sidebarBlue tracking-[0.01em] text-xl leading-[30px] font-semibold">
					...
					>
						<section className="flex flex-col gap-y-6">
							<p className="text-sidebarBlue font-normal text-xs leading-[18px] m-0 p-0 mt-1">
								Votes are calculated by multiplying the votes
								casted by a user with the conviction period.
							</p>
							<p className="font-medium text-xs leading-[18px] text-sidebarBlue m-0 p-0">
								For example:
							</p>
							...
							<p className="p-0 m-0 text-sidebarBlue font-normal text-xs leading-[18px]">
								The vote will be calculated by multiplying{' '}
								<span className="text-pink_primary">
									11.27 KSM (amount) into 4 (conviction) to
									get the final vote.
								</span>
							</p>
						</section>
