...
const ReferendumV2VoteInfo: FC<IReferendumV2VoteInfoProps> = ({
  ...
  useEffect(() => {
    ...
    if (
      ['confirmed', 'executed', 'timedout', 'cancelled', 'rejected'].includes(
        status.toLowerCase(),
      )
    ) {
      setTallyData({
    ...
    (async () => {
      const referendumInfoOf = await api.query.referenda.referendumInfoFor(
        postIndex,
      );
        ...
        setTallyData({
          ...
          support:
            typeof parsedReferendumInfo.ongoing.tally.support === 'string'
              ? new BN(
                  parsedReferendumInfo.ongoing.tally.support.slice(2),
                  'hex',
                )
          ...
          >
            <section className="flex flex-col gap-y-6">
              <p className="text-sidebarBlue font-normal text-xs leading-[18px] m-0 p-0 mt-1">
                Votes are calculated by multiplying the votes casted by a user
                with the conviction period.
              </p>
              ...
              <p className="p-0 m-0 text-sidebarBlue font-normal text-xs leading-[18px]">
                ...
                <span className="text-pink_primary">
                  11.27 KSM (amount) into 4 (conviction) to get the final vote.
                </span>
              </p>
            </section>
          </Modal>
