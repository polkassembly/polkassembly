...
export const getConvictionVoteOptions = (
  CONVICTIONS: [number, number][],
  proposalType: ProposalType,
  ...
  if (
    [ProposalType.REFERENDUM_V2, ProposalType.FELLOWSHIP_REFERENDUMS].includes(
      proposalType,
    ) &&
    ![AllNetworks.COLLECTIVES, AllNetworks.WESTENDCOLLECTIVES].includes(network)
  ) {
    if (api && apiReady) {
      ...
      if (days && !isNaN(Number(days))) {
        return [
          <Select.Option
            className={`text-[#243A57] ${poppins.variable}`}
            key={0}
            value={0}
          >
...
const VoteReferendum = ({
  ...
  const convictionOpts = useMemo(() => {
    return getConvictionVoteOptions(
      CONVICTIONS,
      proposalType,
      api,
      apiReady,
    ...
    if (proposalType === ProposalType.OPEN_GOV) {
      if (vote === EVoteDecisionType.AYE) {
        voteTx = api.tx.convictionVoting.vote(referendumId, {
          Standard: { balance: lockedBalance, vote: { aye: true, conviction } },
        });
    ...
    ? [
        {
          label: (
            <div
              className={`flex items-center justify-center text-[#576D8B] w-[131px] h-[32px] rounded-[4px] ${
                vote === 'aye' ? 'bg-[#2ED47A] text-white' : ''
              }`}
            >
              {vote === EVoteDecisionType.AYE ? (
                <LikeWhite className="mr-2 mb-[3px]" />
              ) : (
                <LikeGray className="mr-2 mb-[3px]" />
              )}
        ...
        },
        {
          label: (
            <div
              className={`flex items-center justify-center text-[#576D8B] w-[126px] h-[32px] rounded-[4px] ${
                vote === 'nay' ? 'bg-[#F53C3C] text-white' : ''
              }`}
            >
              {vote === EVoteDecisionType.NAY ? (
                <DislikeWhite className="mr-2  " />
              ) : (
                <DislikeGray className="mr-2" />
              )}{' '}
        ...
        },
        {
          label: (
            <div
              className={`flex items-center justify-center text-[#576D8B]  w-[126px] h-[32px] rounded-[4px] ${
                vote === 'split' ? 'bg-[#FFBF60] text-white' : ''
              }`}
            >
              {' '}
              {vote === EVoteDecisionType.SPLIT ? (
                <SplitWhite className="mr-2  " />
              ) : (
                <SplitGray className="mr-2" />
              )}{' '}
        ...
        },
        {
          label: (
            <div
              className={` flex items-center justify-center text-[#576D8B] ml-2  w-[126px] h-[32px] rounded-[4px] ${
                vote === 'abstain' ? 'bg-[#407BFF] text-white' : ''
              }`}
            >
              <StopOutlined className="mr-2 mb-[3px]" />{' '}
              <span className="font-medium text-base">Abstain</span>
            </div>
          ),
          value: 'abstain',
        },
    ...
    : [
        {
          label: (
            <div
              className={`flex items-center justify-center text-[#576D8B] w-full h-[32px] ml-1 mr-1 rounded-[4px] ${
                vote === 'aye' ? 'bg-[#2ED47A] text-white' : ''
              }`}
            >
              {vote === EVoteDecisionType.AYE ? (
  ...
  const VoteUI = (
    <>
      <div className={className}>
        ...
        >
          {lastVote == null || lastVote == undefined
            ? 'Cast Vote Now'
            : 'Cast Vote Again'}
        </Button>
        <Modal
          ...
          title={
            <div className="h-[65px] -mt-5 border-0 border-solid border-b-[1.5px] border-[#D2D8E0] mr-[-24px] ml-[-24px] rounded-t-[6px] flex items-center justify-center gap-2">
              ...
              <span className="text-[#243A57] font-semibold tracking-[0.0015em] text-xl">
                Cast Your Vote
              </span>
            </div>
          }
        >
          <>
            ...
            >
              <>
                <div className="text-sm font-normal flex items-center justify-center text-[#485F7D] mt-3">
                  Select a wallet
                </div>
                <div className="flex items-center gap-x-5 mt-1 mb-6 justify-center">
                  {availableWallets[Wallet.POLKADOT] && (
                    <WalletButton
                      className={`${
                        wallet === Wallet.POLKADOT
                          ? ' w-[64px] h-[48px] hover:border-pink_primary border border-solid border-pink_primary'
                          : 'w-[64px] h-[48px]'
                      }`}
                      disabled={!apiReady}
                      onClick={(event) =>
                  ...
                  {(window as any).walletExtension?.isNovaWallet &&
                    availableWallets[Wallet.NOVAWALLET] && (
                      <WalletButton
                        disabled={!apiReady}
                        className={`${
                          wallet === Wallet.NOVAWALLET
                            ? 'border border-solid border-pink_primary  w-[64px] h-[48px]'
                            : 'w-[64px] h-[48px]'
                        }`}
                        onClick={(event) =>
                  ...
                  availableWallets[Wallet.POLYWALLET] ? (
                    <WalletButton
                      disabled={!apiReady}
                      onClick={(event) =>
                ...
                <h3 className="inner-headings mt-[24px] mb-[2px]">
                  Choose your vote
                </h3>
                <Segmented
                  block
                  ...
                  vote !== EVoteDecisionType.ABSTAIN && (
                    ...
                    >
                      <BalanceInput
                        label={'Lock balance'}
                        helpText={
                          'Amount of you are willing to lock for this vote.'
                        }
                      ...
                      <div className="flex justify-end mt-[-3px] pt-5 mr-[-24px] ml-[-24px] border-0 border-solid border-t-[1.5px] border-[#D2D8E0]">
                        <Button
                          className="w-[134px] h-[40px] rounded-[4px] text-[#E5007A] bg-[white] mr-[15px] font-semibold border-[#E5007A]"
                          onClick={() => setShowModal(false)}
                        >
                          Cancel
                        </Button>
                        <Button
                          className={`w-[134px] h-[40px] rounded-[4px] text-[white] bg-[#E5007A] mr-[24px] font-semibold border-0 ${
                            (!wallet || !lockedBalance) && 'opacity-50'
                          }`}
                          htmlType="submit"
