	...
	const decisionOptions = [
		{
			label: (
				<div
					className={`text-[#576D8B] h-[32px] rounded-[4px] ml-1 mr-1 w-full ${
		...
		},
		{
			label: (
				<div
					className={` text-[#576D8B] h-[32px] w-full ml-1 mr-1 rounded-[4px] ${
			...
			>
              ...
              Select a wallet
						...
						{accounts.length > 0 ? (
							...
							<Alert message="Please select a wallet." showIcon type="info" />
						) : null}
						<h3 className="inner-headings mt-6 mb-[2px]">Choose your vote</h3>
						<Segmented
							block
							className={
								'mb-6 border-solid border-[1px] bg-white hover:bg-white border-[#D2D8E0] rounded-[4px] w-full'
							}
							size="large"
							value={vote}
							onChange={(value) => {
						...
						/>
						{
							<Form
								...
								onFinish={async () => {
									vote === EVoteDecisionType.AYE &&
                    (await voteReferendum(true));
									vote === EVoteDecisionType.NAY &&
                    (await voteReferendum(false));
								}}
							>
								<BalanceInput
									label={'Lock balance'}
                    ...
                    Cancel
									...
									<Button
										className={`w-[134px] h-[40px] rounded-[4px] text-[white] bg-[#E5007A] mr-[24px] font-semibold border-0 ${
											(!lockedBalance || !wallet) && 'opacity-50'
										}`}
										htmlType="submit"
