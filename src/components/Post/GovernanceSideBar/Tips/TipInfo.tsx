...
const getMedian = (list: ITippersInfo[], members: string[], proposer?: string, receiver?: string): BN => {
	let median = new BN(0);
	if (list.length > 0) {
		const values: BN[] = list.map((info) => new BN(info.value)).sort((a, b) => new BN(a).cmp(new BN(b)));
		const midIndex = Math.floor(values.length / 2);
		median = values.length ? (values.length % 2 ? values[midIndex] : new BN(values[midIndex - 1]).add(new BN(values[midIndex])).divn(2)) : new BN(0);
		if (proposer && receiver && proposer !== receiver && !members.includes(proposer) && median != new BN(0)) {
