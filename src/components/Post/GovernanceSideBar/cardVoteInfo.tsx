...
import RHSVoteCardSlides from '~src/components/RHSVoteCardSlides';

interface ICardVoteInfoProps {
...
const CardVoteInfo: FC<ICardVoteInfoProps> = (props) => {
	...
	const { created_at, track_number, statusHistory, postIndex, postType } = post;
	...
	const handleCurveData = async () => {
		...
		if (tracks && Array.isArray(tracks)) {
			...
			if (track && Array.isArray(track) && track.length > 1) {
				...
				const statusBlock = statusHistory?.find((s: any) => s?.status === 'Deciding');
				if (statusBlock) {
...
export default styled(memo(CardVoteInfo))`
	.edit-icon-wrapper {
		transition: all 0.5s;
	}
