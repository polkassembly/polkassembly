...
export function getReferendumVotingFinishHeight(
  ...
  if (timeline && Array.isArray(timeline) && timeline.length > 0) {
    ...
    if (
      singleTimeline &&
      singleTimeline.statuses &&
      Array.isArray(singleTimeline.statuses)
    ) {
      const finishItem = singleTimeline.statuses.find((obj: any) =>
        [
          'Approved',
          'Rejected',
          'TimedOut',
...
const GovernanceSideBar: FC<IGovernanceSidebarProps> = (props) => {
  ...
  useEffect(() => {
    if (
      [ProposalType.OPEN_GOV, ProposalType.FELLOWSHIP_REFERENDUMS].includes(
        proposalType,
      )
    ) {
      if (!api || !apiReady) {
      ...
      const getData = async () => {
        const tracks =
          network != 'collectives'
            ? api.consts.referenda.tracks.toJSON()
        ...
        if (tracks && Array.isArray(tracks)) {
          ...
          if (track && Array.isArray(track) && track.length > 1) {
            ...
            if (
              subsquidRes &&
              subsquidRes.data &&
              subsquidRes.data.curveData &&
              Array.isArray(subsquidRes.data.curveData)
            ) {
              ...
              if (graph_points?.length > 0) {
                ...
                graph_points?.forEach((graph_point: any) => {
                  const hour = dayjs(graph_point.timestamp).diff(
                    proposalCreatedAt,
                    'minute',
                  );
                ...
                const currentApproval =
                  currentApprovalData[currentApprovalData.length - 1];
                const currentSupport =
                  currentSupportData[currentSupportData.length - 1];

                setProgress({
                  approval: currentApproval?.y?.toFixed(1) as any,
                  approvalThreshold:
                    (approvalData.find(
                      (data) => data && data?.x >= currentApproval?.x,
                    )?.y as any) || 0,
                  support: currentSupport?.y?.toFixed(1) as any,
                  supportThreshold:
                    (supportData.find(
                      (data) => data && data?.x >= currentSupport?.x,
                    )?.y as any) || 0,
                });
              }
            } else {
              setCurvesError(
                subsquidRes.errors?.[0]?.message || 'Something went wrong.',
              );
            }
  ...
  useEffect(() => {
    if (trackInfo) {
      ...
      const { approvalCalc, supportCalc } = getTrackFunctions(trackInfo);
      if (
        typeof approvalCalc === 'function' &&
        typeof supportCalc === 'function'
      ) {
  ...
  const LastVoteInfoOnChain: FC<IVoteHistory> = ({
    ...
    return (
      <Spin spinning={isLastVoteLoading} indicator={<LoadingOutlined />}>
        <p className="font-medium text-[12px] leading-6 text-[#243A57] mb-[5px]">
          Last Vote:
        </p>
          ...
          >
            <span className="h-[25px]">
              {decision == 'yes' ? (
                <p>
                  <AyeGreen />{' '}
                  <span className="capitalize font-medium text-[#2ED47A]">
                    {'Aye'}
                  </span>
                </p>
              ) : decision == 'no' ? (
                <p>
                  <DislikeIcon className="text-[#F53C3C]" />{' '}
                  <span className="mb-[5px] capitalize font-medium text-[#F53C3C]">
                    {'Nay'}
                  </span>
                </p>
              ) : decision == 'abstain' && !(balance as any).abstain ? (
                <p>
                  <SplitYellow className="mb-[-2px]" />{' '}
                  <span className="capitalize font-medium text-[#FFBF60]">
                    {'Split'}
                  </span>
                </p>
              ) : decision == 'abstain' && (balance as any).abstain ? (
                <p className="flex justify-center align-middle">
                  ...
                  <span className="capitalize font-medium  text-[#243A57]">
                    {'Abstain'}
                  </span>
                </p>
              ) : null}
          ...
          <Form>
            ...
            {canEdit &&
              ...
              !(
                ...
                <div className=" rounded-[14px] bg-white shadow-[0px_6px_18px_rgba(0,0,0,0.06)] pb-[36px] mb-8">
                  <div
                    className="flex justify-end py-[17px] px-[20px] items-center"
            ...
            {accountsNotFound || extensionNotFound ? (
              <GovSidebarCard>
                {accountsNotFound ? (
                  <div className="mb-4">
                    <p className="mb-4">
                      You need at least one account in Polkadot-js extension to
                      use this feature.
                    </p>
            ...
            {proposalType === ProposalType.COUNCIL_MOTIONS && (
              <>
                ...
                {post.motion_votes && (post.motion_votes?.length || 0) > 0 && (
                  <MotionVoteInfo councilVotes={post.motion_votes} />
                )}
              </>
            )}
              ...
              <>
                {proposalType === ProposalType.REFERENDUMS ? (
                  <>
                    {canVote && (
                      <>
                        {['moonbase', 'moonbeam', 'moonriver'].includes(
                          network,
                        ) ? (
                          <>
                            {metaMaskError &&
                              !walletConnectProvider?.wc.connected && (
                                <GovSidebarCard>{metaMaskError}</GovSidebarCard>
                              )}

                            {(!metaMaskError ||
                              walletConnectProvider?.wc.connected) && (
                              <GovSidebarCard className="overflow-y-hidden">
                                <h6 className="text-bodyBlue font-medium text-xl mx-0.5 mb-6 leading-6">
                                  Cast your Vote!
                                </h6>
                                <VoteReferendumEth
                  ...
                  <>
                    ...
                    <ReferendaV2Messages progress={progress} />
                        ...
                        {proposalType === ProposalType.OPEN_GOV && (
                          <div className={className}>
                            ...
                            <Modal
                              ...
                              closeIcon={<CloseIcon />}
                              title={
                                <h2 className="text-bodyBlue tracking-[0.01em] text-xl leading-[30px] font-semibold">
                                  Threshold Curves
                                </h2>
                              }
                            >
                ...
                <div>
                  {lastVote != undefined ? (
                    lastVote == null ? (
                      <GovSidebarCard>
                        You haven&apos;t voted yet, vote now and do your bit for
                        the community
                      </GovSidebarCard>
                    ) : (
                      <></>
