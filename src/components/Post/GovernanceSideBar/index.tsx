			...
			const getData = async () => {
				...
				if (tracks && Array.isArray(tracks)) {
					...
					if (track && Array.isArray(track) && track.length > 1) {
						...
						const strArr = blockToTime(decisionPeriod, network)[
							'time'
						].split(' ');
						...
						const currentSupportData: { x: number; y: number }[] =
							[];
						const { approvalCalc, supportCalc } =
							getTrackFunctions(trackInfo);
						...
						if (
							...
							if (graph_points?.length > 0) {
								...
								const currentApproval =
									currentApprovalData[
										currentApprovalData.length - 1
									];
								const currentSupport =
									currentSupportData[
										currentSupportData.length - 1
									];
						...
						{accountsNotFound || extensionNotFound ? (
							<GovSidebarCard>
								{accountsNotFound ? (
									<div className="mb-4">
										<p className="mb-4">
											You need at least one account in
											Polkadot-js extension to use this
											feature.
										</p>
										<p className="text-muted m-0">
											Please reload this page after adding
											accounts.
										</p>
									</div>
								) : null}
