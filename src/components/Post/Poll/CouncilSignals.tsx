			...
			{councilVotes.length > 0 ? (
				<GovSidebarCard className={className}>
					<h3 className="flex items-center">
						<span className="mr-2 dashboard-heading">
							Council Signals
						</span>{' '}
						<HelperTooltip text="This represents the off-chain votes of council members" />
					</h3>
								...
								format={() =>
									ayes && nays ? (
										<div className="text-sm">
											<div className="text-green-400 border-b border-b-gray-400 mx-10">
												{(
													(ayes / (ayes + nays)) *
													100
												).toFixed(1)}
											...
											<div className="text-pink_primary">
												{(
													(nays / (ayes + nays)) *
													100
												).toFixed(1)}
					...
					{councilVotes.length > 0 && (
						<div>
							...
							{councilVotes.map((councilVote) => (
								<div
									className="flex items-center px-12 mt-3"
									key={councilVote.address}
								>
									<div className="mr-auto">
										<Address
											address={councilVote.address}
										/>
									</div>
									<div>
