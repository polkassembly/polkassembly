...
const Post: FC<IPostProps> = (props) => {
  ...
  useEffect(() => {
    ...
    let isProposer =
      proposer &&
      addresses?.includes(getSubstrateAddress(proposer) || proposer);
    ...
    (async () => {
      ...
      const { data, error: fetchError } = await nextApiClientFetch<IVerified>(
      ...
      if (fetchError || !data)
        return console.log('error checking verified address : ', fetchError);
  ...
  useEffect(() => {
    if (
      ...
      dayjs(post.created_at).isBefore(dayjs().subtract(6, 'days'))
    )
      return;
  ...
  const productData = async () => {
    try {
      const response = await fetch(
        `https://api.github.com/repos/CoinStudioDOT/OpenGov/contents/${networkModified}/${postType}/${
          postType === ProposalType.TIPS ? post.hash : post.post_id
        }`,
        {
          headers: {
        ...
        {isOffchainPost && (
          <>
            ...
            <OptionPoll
              proposalType={proposalType}
              postId={
                proposalType === ProposalType.TIPS ? post.hash : post.post_id
              }
              canEdit={post.user_id === id}
            />
          </>
  ...
  const TrackerButtonComp = (
    <>
      {id && !isNaN(Number(onchainId)) && isOnchainPost && !isEditing && (
        <TrackerButton onchainId={onchainId} proposalType={proposalType} />
      )}
    </>
  );
  ...
  const getOnChainTabs = () => {
    ...
    if (['polkadot', 'kusama'].includes(network)) {
      tabs.push({
        children: <PostAudit auditData={auditData} videoData={videoData} />,
        key: 'audit',
        label: (
          <div className="flex gap-2 items-center justify-center audit">
            Audit
            {totalAuditCount + totalVideoCount > 0 && (
              <span className="bg-[#d6d8da] card-bg text-xs font-medium rounded-full px-1.5 text-bodyBlue py-0.5">
    ...
    >
      <>
        ...
        {!isEditing &&
          ...
          (post?.timeline.length === 1
            ? getFirestoreProposalType(post?.timeline[0]?.type) !== proposalType
            : true) && (
            <LinkCard timeline={post?.timeline} proposalType={proposalType} />
          )}
        {proposalType === ProposalType.CHILD_BOUNTIES &&
          (post.parent_bounty_index || post.parent_bounty_index === 0) && (
            <Link href={`/bounty/${post.parent_bounty_index}`}>
              <div className="bg-white drop-shadow-md p-3 md:p-6 rounded-md w-full mb-6 dashboard-heading">
                This is a child bounty of{' '}
                <span className="text-pink_primary">
                  Bounty #{post.parent_bounty_index}
                </span>
              </div>
            </Link>
          )}
        ...
        <div className={`${className} grid grid-cols-1 xl:grid-cols-12 gap-9`}>
          <div className="xl:col-span-8">
            {proposalType === ProposalType.GRANTS &&
              dayjs(post.created_at).isAfter(dayjs().subtract(6, 'days')) && (
                <div className="bg-white drop-shadow-md p-3 md:p-6 rounded-md w-full mb-6 dashboard-heading">
