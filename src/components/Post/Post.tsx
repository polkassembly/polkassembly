					...
					<PostOnChainInfo
						onChainInfo={{
							...
							submission_deposit_amount:
								post?.submission_deposit_amount,
			...
			<>
				...
				{!isEditing &&
					...
					(post?.timeline.length === 1
						? getFirestoreProposalType(post?.timeline[0]?.type) !==
						  proposalType
						: true) && (
						<LinkCard
							timeline={post?.timeline}
				...
				{proposalType === ProposalType.CHILD_BOUNTIES &&
					(post.parent_bounty_index ||
						post.parent_bounty_index === 0) && (
				...
				{post &&
					...
					postStatus === 'PendingPayout' && (
						<div className="bg-white drop-shadow-md p-3 md:p-6 rounded-md  mb-6 dashboard-heading flex items-center gap-x-2 w-full">
							<span>
								The child bounty payout is ready to be claimed
							</span>
							<ClaimPayoutModal
				...
				<div
					className={`${className} grid grid-cols-1 xl:grid-cols-12 gap-9`}
				>
