...
type props = { post: any; canEdit: any; showDecisionDeposit: any; trackName: string; toggleEdit: (() => void) | null };
const RHSVoteCardSlides = ({ canEdit, showDecisionDeposit, trackName, toggleEdit, post }: props) => {
	...
	const { post_link, tags, postType, content, statusHistory, postIndex } = post;
	...
	useEffect(() => {
		...
		(async () => {
			if (
				!statusHistory?.filter((status: any) =>
...
export default RHSVoteCardSlides;
