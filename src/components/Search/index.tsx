...
const NewSearch = ({
    ...
    const handleSearchOnChange = async (queryStr: string) => {
        ...
        if (
            ...
            autoCompleteResults.users.length
        ) {
            setAutoCompleteResults(initAutocompleteResults);
        } else {
    ...
    function getMatchedWordsLength(hitObject: any) {
        ...
        matchedWordsLength +=
            hitObject._highlightResult?.pasrsed_content?.matchedWords?.length ||
            0;
        matchedWordsLength +=
        ...
        matchedWordsLength +=
            hitObject._highlightResult?.profile?.title?.matchedWords?.length ||
            0;
    ...
    const dedupedSortedAutoCompleteResults = useMemo(() => {
        ...
        for (const [index, item] of sortedAutoCompleteResults.entries()) {
            ...
            for (const result of sortedAutoCompleteResults.slice(0, index)) {
                const resultStr =
                    getAutocompleteMarkedText(result._highlightResult) ||
                    'No title';
                if (resultStr === str) {
                    isDuplicate = true;
                    break;
            ...
            >
                ...
                <div
                    className={`mt-[18px] flex justify-between max-md:flex-col max-md:gap-2 radio-btn ${
                        isSuperSearch &&
                        'max-lg:flex-col max-lg:gap-2 flex-wrap'
                    }`}
                >
                    <Radio.Group
