...
const NewSearch = ({
  ...
  const [selectedOpengovTracks, setSelectedOpengovTracks] = useState<
    CheckboxValueType[]
  >([]);
  const [selectedGov1Tracks, setSelectedGov1Tracks] = useState<
    CheckboxValueType[]
  >([]);
  ...
  const [onchainPostResults, setOnchainPostResults] = useState<{
    data: any[];
    total: number;
  } | null>(null);
  const [offchainPostResults, setOffchainPostResults] = useState<{
    data: any[];
    total: number;
  } | null>(null);
    ...
    return [
      ...
      selectedTopics.map(
        (topic) =>
          `topic_id:${
            post_topic[
              optionTextToTopic(String(topic)) as keyof typeof post_topic
            ]
          }`,
      ),
    ];
  ...
  const getpeopleDatawithAddress = (addressData: any[], postsData: any[]) => {
    ...
    const results = postsData?.map((people: any) => {
      ...
      for (const data of addressData) {
        if (
          Number(data?.objectID) === Number(data?.objectID) &&
          data?.default
        ) {
  ...
  function getMatchedWordsLength(hitObject: any) {
    ...
    matchedWordsLength +=
      hitObject._highlightResult?.title?.matchedWords?.length || 0;
    matchedWordsLength +=
      hitObject._highlightResult?.pasrsed_content?.matchedWords?.length || 0;
    matchedWordsLength +=
      hitObject._highlightResult?.username?.matchedWords?.length || 0;
    matchedWordsLength +=
      hitObject._highlightResult?.profile?.bio?.matchedWords?.length || 0;
    matchedWordsLength +=
      hitObject._highlightResult?.profile?.title?.matchedWords?.length || 0;
  ...
  const getAutocompleteMarkedText = (highlightResult: {
    [index: string]: any;
  }) => {
    ...
    for (const key of keysToCheck) {
      ...
      if (
        result &&
        result.matchedWords &&
        result.matchedWords.length > maxMatchedWordsLength
      ) {
  ...
  const sortedAutoCompleteResults = useMemo(() => {
    return autoCompleteResults.posts
      .concat(autoCompleteResults.users)
      .sort((a, b) => {
  ...
  const dedupedSortedAutoCompleteResults = useMemo(() => {
    ...
    for (const [index, item] of sortedAutoCompleteResults.entries()) {
      ...
      const str =
        getAutocompleteMarkedText(item._highlightResult) || 'No title';
      ...
      for (const result of sortedAutoCompleteResults.slice(0, index)) {
        const resultStr =
          getAutocompleteMarkedText(result._highlightResult) || 'No title';
        if (resultStr === str) {
          isDuplicate = true;
          break;
        }
  ...
  return (
    <Modal
      title={
        <label
          className={
            'text-bodyBlue text-xl font-semibold flex flex-wrap search gap-1'
          }
        >
          {isSuperSearch ? (
            <span
    ...
    >
      <div className={`${className} ${isSuperSearch && !loading && 'pb-2'}`}>
        <Input
          ...
          addonAfter={
            <div
              ...
              className={`text-white text-[18px] tracking-[0.02em]  ${
            ...
            <section className="border-solid border-[1px] border-gray-200 rounded-b-[4px] absolute z-50 w-[94.3%] max-md:w-[85.7%] bg-white">
              ...
              <List
                ...
                renderItem={(item) => {
                  ...
                  const str =
                    getAutocompleteMarkedText(item._highlightResult) ||
                    'No title';
                  const cleanStr = getCleanString(str);

                  return (
                    <List.Item
                      className="flex justify-start flex-wrap hover:cursor-pointer whitespace-nowrap hover:bg-[#FEF2F8] "
                      onClick={() => {
                        setFilterBy(
                          !isPost
                            ? EFilterBy.People
                            : item.post_type === 'discussions'
                    ...
                    >
                      <Markdown
                        className="hover:text-pink_primary flex flex-wrap max-md:truncate"
                        md={str}
                        isAutoComplete
                      />
                      <span className="text-[9px] mx-2 text-gray-400">
                        &#9679;
                      </span>
                      <span className="text-xs text-gray-500">
                        {isPost ? 'in Posts' : 'in People'}
                      </span>
                    </List.Item>
                  );
                }}
        ...
        <div
          className={`mt-[18px] flex justify-between max-md:flex-col max-md:gap-2 radio-btn ${
            isSuperSearch && 'max-lg:flex-col max-lg:gap-2 flex-wrap'
          }`}
        >
          <Radio.Group
          ...
          >
            <Radio
              ...
              className={`text-xs font-medium py-1.5 rounded-[24px] ${
                filterBy === EFilterBy.Referenda && finalSearchInput.length > 0
                  ? 'bg-[#FEF2F8] text-bodyBlue md:px-2'
                  : 'text-[#667589]'
              } ${
                finalSearchInput.length === 0 && 'text-[#B5BFCC]'
              } max-sm:text-[10px]`}
            >
              Referenda{' '}
              {finalSearchInput.length > 0 &&
                `(${onchainPostResults?.total || 0})`}
            </Radio>
            <Radio
            ...
            >
              Discussions{' '}
              {finalSearchInput.length > 0 &&
                `(${offchainPostResults?.total || 0})`}
            </Radio>
          </Radio.Group>
          {(filterBy === EFilterBy.Referenda ||
            filterBy === EFilterBy.Discussions) && (
            <div className="flex text-xs font-medium tracking-[0.02em] text-[#667589] gap-3.5 max-md:px-0 max-md:gap-1.5">
              {isSuperSearch && (
                <NetworkDropdown
                  setSidedrawer={() => {}}
                  isSmallScreen={true}
              ...
              <Popover
                ...
                onOpenChange={() =>
                  finalSearchInput.length > 0 &&
                  setOpenFilter({ ...openFilter, date: !openFilter.date })
                }
                content={
                  <div className="flex flex-col gap-1">
                    <Radio.Group
                      ...
                      value={dateFilter}
                      className={`gap-[1px] flex flex-col ${poppins.variable} ${poppins.className}`}
                    >
                      <Radio
                        value={EDateFilter.Today}
                        className={`text-xs font-normal py-1.5 ${
                      ...
                      <Radio
                        ...
                        className={`text-xs font-normal py-1.5 ${
                          dateFilter === EDateFilter.Last_7_days
                            ? 'text-bodyBlue'
                            : 'text-[#667589]'
                        }`}
                      >
                      ...
                      <Radio
                        ...
                        className={`text-xs font-normal py-1.5 ${
                          dateFilter === EDateFilter.Last_30_days
                            ? 'text-bodyBlue'
                            : 'text-[#667589]'
                        }`}
                      >
                      ...
                      <Radio
                        ...
                        className={`text-xs font-normal py-1.5 ${
                          dateFilter === EDateFilter.Last_3_months
                            ? 'text-bodyBlue'
                            : 'text-[#667589]'
                        }`}
                      >
                      ...
                      <Radio
                        value={null}
                        className={`text-xs font-normal py-1.5 ${
                          !dateFilter ? 'text-bodyBlue' : 'text-[#667589]'
                        }`}
                      >
                        All time
                      </Radio>
              ...
              >
                <div
                  className={`flex items-center justify-center text-xs ${
                    openFilter.date && 'text-pink_primary'
                  } ${
                    finalSearchInput.length === 0
                ...
                >
                  ...
                  <span className="text-[#96A4B6]">
                    {openFilter.date ? (
                      <HighlightDownOutlined className="ml-2.5 mt-1 max-md-ml-1" />
                    ) : (
                      <DownOutlined className="ml-2.5 mt-1 max-md-ml-1" />
                    )}
                  </span>
                </div>
              </Popover>
              ...
              {filterBy === EFilterBy.Referenda && (
                <Popover
                  ...
                  onOpenChange={() =>
                    finalSearchInput.length > 0 &&
                    setOpenFilter({ ...openFilter, track: !openFilter.track })
                  }
                  content={
                    <Collapse
                      collapsible="header"
                            ...
                            gov1Tracks?.map((track) => (
                              <Checkbox
                                ...
                                className={`text-xs font-normal py-1.5 ml-0 ${
                                  selectedGov1Tracks.includes(track)
                                    ? 'text-bodyBlue'
                                    : 'text-[#667589]'
                                }`}
                              >
                          ...
                          {openGovTracks &&
                            openGovTracks?.map((track) => (
                              <Checkbox
                                ...
                                value={track?.trackId}
                                className={`text-xs font-normal py-1.5 ml-0 ${
                                  selectedOpengovTracks.includes(track?.name)
                                    ? 'text-bodyBlue'
                                    : 'text-[#667589]'
                                }`}
                              >
                ...
                >
                  <div
                    className={`flex items-center justify-center text-xs ${
                      openFilter.track && 'text-pink_primary'
                    } ${
                      finalSearchInput.length === 0
                        ? 'text-[#B5BFCC] cursor-not-allowed'
                        : 'cursor-pointer'
              ...
              <Popover
                ...
                onOpenChange={() =>
                  finalSearchInput.length > 0 &&
                  setOpenFilter({ ...openFilter, topic: !openFilter.topic })
                }
                content={
                  <Checkbox.Group
                    className={`checkboxStyle flex flex-col tracking-[0.01em] justify-start ${poppins.className} ${poppins.variable}`}
              ...
              >
                <div
                  className={`flex items-center justify-center text-xs ${
                    openFilter.topic && 'text-pink_primary'
                  } ${
                    finalSearchInput.length === 0
                      ? 'text-[#B5BFCC] cursor-not-allowed'
                      : 'cursor-pointer'
        ...
        {filterBy !== EFilterBy.People && isFilter && (
          <div className="mt-3 flex flex-wrap justify-between text-xs font-medium text-bodyBlue ">
            <div className="flex gap-1 max-sm:mb-2 max-sm:flex-wrap">
              {isSuperSearch && selectedNetworks.length > 0 && (
                <div className="py-1 px-2 bg-[#FEF2F8] flex gap-1 rounded-[4px]">
              ...
              {dateFilter && (
                <div className="py-1 px-2 bg-[#FEF2F8] flex gap-1 rounded-[4px] font-medium">
                  <span className="text-pink_primary">Date:</span>
                  <span className="capitalize">
                    {dateFilter?.split('_')?.join(' ')}
                  </span>
                </div>
              )}
              {selectedTags.length > 0 && (
                    ...
                    <span className="flex flex-wrap">
                      {selectedOpengovTracks?.map((trackId, index) => (
                        <span key={index} className="capitalize">
                          {getTrackNameFromId(network, Number(trackId))
                            ?.split('_')
                            ?.join(' ')}
                      ...
                      {selectedGov1Tracks.map((track, index) => (
                        <span
                          key={index}
                          className="capitalize flex flex-shrink-0"
                        >
                          {' '}
                          {selectedOpengovTracks.length > 0 && ', '}
            ...
            {finalSearchInput.length > 0 && (
              <span
                className={`${
                  !isFilter
                    ? 'text-[#667589] cursor-default'
                    : 'text-pink_primary cursor-pointer'
                } max-sm:border-solid max-sm:border-[1px] flex max-sm:border-pink_primary max-sm:p-1 items-center`}
        ...
        {(finalSearchInput.length > 2 || searchInputErr.err) && (
          <div className={`${loading && 'hidden'} z-10`}>
            {(filterBy === EFilterBy.Referenda ||
              filterBy === EFilterBy.Discussions) &&
              !searchInputErr.err &&
              (onchainPostResults || offchainPostResults) && (
                <ResultPosts
                  setOpenModal={setOpenModal}
                  isSuperSearch={isSuperSearch}
                  postsData={
                    filterBy === EFilterBy.Referenda
                      ? onchainPostResults?.data || []
                      : offchainPostResults?.data || []
                  }
                  totalPage={
                    filterBy === EFilterBy.Discussions
                      ? offchainPostResults?.total || 0
            ...
            {!loading &&
              (searchInputErr.err ||
                ...
                <SearchErrorsCard
                  isSearchErr={
                    searchInput?.trim().length <= 2 && searchInputErr.clicked
                      ? true
                      : searchInputErr?.err
                  }
                  filterBy={filterBy}
        ...
        <div
          className={`flex flex-col justify-center items-center pt-10 pb-8 gap-4 ${
            !loading && 'hidden'
          }`}
        >
          ...
          <span className="font-medium text-sm text-bodyBlue tracking-[0.01em] text-center">
            {isSuperSearch
              ? 'Looking for results across the Polkassembly Universe.'
              : 'Looking for results.'}
          </span>
        </div>

        {finalSearchInput.length === 0 && justStart && (
          <div className="h-[360px] flex justify-center items-center flex-col font-medium text-sm text-bodyBlue">
            <StartSearchIcon />
            <span className="mt-8 tracking-[0.01em] text-center">
              Welcome to the all new & supercharged search!
            </span>
            <div className="text-xs font-medium mt-2 tracking-[0.01em] flex gap-1 items-center">
              powered by
              <PaLogo className="w-[99px] h-[30px]" />
            </div>
          </div>
        )}
      </div>
...
export default styled(NewSearch)`
  ...
  .supersearch:hover {
    filter: brightness(0) saturate(100%) invert(13%) sepia(94%) saturate(7151%)
      hue-rotate(321deg) brightness(90%) contrast(101%);
  }
`;
