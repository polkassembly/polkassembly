...
import { Checkbox, Modal, Popover, Radio, RadioChangeEvent } from 'antd';
...
function ProductItem({ hit, components }) {
	return (
		<a href={hit.url} className="aa-ItemLink border-solid w-full">
			<div className="aa-ItemContent">
				<div className="aa-ItemTitle">
					<components.Highlight hit={hit} attribute="username" />
				</div>
			</div>
		</a>
	);
...
const Search = ({ className, openModal, setOpenModal, isSuperSearch, setIsSuperSearch }: Props) => {
	...
	const getResultData = async() => {
		...
		if(filterBy !== EFilterBy.Users){
			...
			const topicFilter = [selectedTopics.map((topic) => {
				...
				return `${filterBy === EFilterBy.Referenda ? 'topic.id' : 'topic_id'}:${post_topic[topicStr as keyof typeof post_topic]}`;
			} )];
	...
	const debouncedSearchFn = useCallback(_.debounce(getResultData, 5000), [searchInput, selectedTags, filterBy, selectedTopics, usersPage.page, postsPage.page, isSuperSearch]);
		...
		<div className={`${className} ${isSuperSearch && !loading && 'pb-20'}`}>
			...
			{!loading ?
				...
				searchInput.length > 2 && (postResults || userResults) && <>
					<div className='mt-[18px] flex justify-between max-md:flex-col max-md:gap-2'>
						...
						{(filterBy === EFilterBy.Referenda || filterBy === EFilterBy.Discussions) && <div className='flex text-xs font-medium tracking-[0.02em] text-[#667589] gap-3.5 max-md:px-4'>
							...
							<Popover content={<Checkbox.Group className={`checkboxStyle flex flex-col tracking-[0.01em] justify-start ${poppins.className} ${poppins.variable}`} onChange={(list) => handleFilterChange(list, EMultipleCheckFilters.Topic)} value={selectedTopics} >
