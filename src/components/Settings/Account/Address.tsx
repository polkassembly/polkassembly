  ...
  which: Wallet;
	...
	return (
		<div className="flex items-center gap-x-2">
			...
			<h4 className="capitalize m-0">{which.replace('-', ' ')}</h4>
		</div>
	);
};
...
const Address: FC<Props> = ({ dismissModal, open }) => {
  ...
  const handleLink = async (
  	...
  	const signRaw =
      !address.startsWith('0x') && signersMap[accountsMap[address]].signRaw;
  	if (!address.startsWith('0x') && !signRaw)
  	...
  	if (substrate_address.startsWith('0x')) {
  		...
  		(window as any).web3.currentProvider.sendAsync(
            ...
            await nextApiClientFetch<ChangeResponseType>(
            	'api/v1/auth/actions/addressLinkConfirm',
            	{
            		address: substrate_address,
            		signature,
            		wallet
  ...
  const UnlinkButton: FC<{ address: string }> = ({ address }) => {
  	const StyledUnlinkButton: FC<{ withClickHandler?: boolean }> = ({
  		...
  		return (
  			<Button
  				className={`font-medium text-sm m-0 p-0 outline-none border-none flex items-center justify-center text-red_primary ${
  ...
  const addressList = ({ accounts, title }: AccountsDetails) => {
  	const accountsObj = accounts.reduce(
  		(prev, account) => {
  			...
  			const encodedUserAddresses = currentUser.addresses?.map((address) =>
  	...
  	return (
  		<article className="flex flex-col gap-y-2">
  			<label className="font-medium text-sm tracking-wide text-sidebarBlue">
  				{title}
  			</label>
            										...
            										<>
            											<div className="col-span-1">
            												<Button
            													className="font-medium text-sm m-0 p-0 text-grey_primary outline-none border-none flex items-center justify-center"
            													onClick={() =>
            														handleLink(address, key as Wallet)
            													}
            													icon={<LinkOutlined />}
            												>
                                      Link
  						...
  						<Button
  							...
  							onClick={() => {
  								getAllAccounts({
  									api,
  									apiReady,
  									get_erc20: ['moonbase', 'moonriver', 'moonbeam'].includes(
  										network
  									),
  									network
  								})
