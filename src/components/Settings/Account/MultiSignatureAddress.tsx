...
const MultiSignatureAddress: FC<Props> = ({ open, dismissModal }) => {
	...
	const [signatoryAccounts, setSignatoryAccounts] = useState<
  ...
  >([]);
		...
		return (
			<>
				{signatoryAccounts.map((account) => {
					...
					return (
						address && (
							<div key={address} className="flex items-center gap-x-2">
								<Checkbox
	...
	const handleSign = async (
      ...
      await nextApiClientFetch<ChangeResponseType>(
      	'api/v1/auth/actions/multisigLinkConfirm',
      	{
      		address: substrate_address,
		...
		>
			...
			>
				<section className="flex flex-col gap-y-4 w-full">
					<label className="flex items-center gap-x-3 text-sm text-sidebarBlue font-normal tracking-wide leading-6">
            Signatory Addresses
						<HelperTooltip
							placement="right"
					...
					{showSignatoryAccounts && signatoryAccounts.length > 0 && (
