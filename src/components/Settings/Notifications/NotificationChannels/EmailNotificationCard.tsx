		...
		<div className="flex flex-col mb-2">
			<h3 className="flex gap-2 items-center text-base text-[#243A57] font-medium m-0 gap-1">
				<span>
					...
					{!verified && (
						<span className="text-[10px] px-[4px] py-[2px] bg-[red] border-[#5A46FF] border-2 text-[#FFFFFF] rounded-tr-lg rounded-bl-lg">
							{' '}
					...
					<span className="flex gap-1 items-center">
						<Switch
							...
							onChange={(checked) =>
								!checked
									? handleToggleClick()
									: handleEnableDisabled(CHANNEL.EMAIL, true)
							}
						/>
						<label>
							<span
								className={`text-[14px] font-medium  ${
									notificationEnabled ? 'text-pink_primary' : 'text-[#485F7D]'
								}`}
							>
			...
			<div className="ml-5">
				<h3 className="m-0 text-[#243A57] text-[14px]">
					{verifiedEmail
						? verifiedEmail
						: 'Please add your email on account page.'}
				</h3>
			</div>
