...
const ChangeEmail: FC<IEmailProps> = ({
  disabled,
  value,
  name,
  label,
...
const Profile = () => {
  ...
  const isSubmitDisabled = web3signup
    ? email === ''
    : isChange
    ? !passwords.old || !passwords.new
    : !currentPassword || currentEmail === email;
    ...
    <Row className="flex flex-col w-full">
      ...
      <Form form={form} onFinish={handleSubmit} className="mt-8">
        ...
        <Button
          ...
          } mt-5 rounded-lg font-semibold text-lg leading-7 text-white py-3 outline-none border-none px-14 flex items-center justify-center bg-pink_primary`}
        >
          Save
        </Button>
      </Form>
