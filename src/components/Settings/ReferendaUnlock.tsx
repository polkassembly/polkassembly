...
const ReferendaUnlock: FC<IReferendaUnlockProps> = ({
	...
	const getLockedBalance = async () => {
		...
		const votes: Vote[] = [];
		const votingInfoEntries =
			await api.query.convictionVoting.votingFor.entries(address);
						...
						<Form.Item>
							...
							{accounts.length > 0 ? (
								...
								<span className="text-sidebarBlue">
									No accounts found, Please approve request
									from your wallet and/or{' '}
										...
										{votes.map((vote) => (
											<>
												<li
													...
													className="grid grid-cols-6 md:grid-cols-8 gap-x-5 py-1"
												>
													<span className="col-span-2">
														<Link
