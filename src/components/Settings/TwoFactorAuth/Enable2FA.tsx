...
const Enable2FA: FC<{ className?: string }> = ({ className }) => {
	...
	const [tfaResponse, setTfaResponse] =
		useState<I2FAGenerateResponse>(init2FARes);
	...
	const handleSubmit = async (formData: any) => {
		...
		if (loading || !currentUser?.username || currentUser.is2FAEnabled)
			return;
		...
		>
				...
				{!currentUser.is2FAEnabled ? (
					...
					>
							...
							!loading && (
								<div className="mb-4">
									<FilteredError
										text={
											error ||
											'Error in generating two factor auth QR Code. Please reload and try again.'
										}
									/>
								</div>
							)}
						...
						<section className="flex flex-col">
							...
							<article>
								<h2 className="text-base text-sidebarBlue">
								...
								<ol className="ml-4">
									...
									<li className="mb-1">
										In the authenticator app, select the
										&quot;+&quot; icon.
									</li>
									<li className="mb-1">
										Select &quot;Scan a QR code&quot; and
							...
							<article className="mt-4">
								<h2 className="text-base text-sidebarBlue">
									Or Enter the Code to Your App (base32
									encoded) :
								</h2>
								{tfaResponse.base32_secret && (
									<span
			...
			>
				...
				<span className="block text-[14px]">
					...
					Verify your identity with an extra step for added
					protection.{' '}
				</span>
			</Button>
		</Form>
