...
const WalletButtons = dynamic(
	() => import('~src/components/Login/WalletButtons'),
	{
		loading: () => (
...
const Web2Signup: FC<Props> = ({
	...
	return (
		<article
			className={`bg-white shadow-md rounded-md p-8 flex flex-col gap-y-6 ${className}`}
		>
			<div className="grid grid-cols-2">
				<div
					...
					className={`cursor-pointer font-medium text-grey_primary flex flex-col gap-y-2 text-xs justify-center items-center sm:flex-row sm:text-sm gap-x-2 border-b-2 pb-2 ${
						!isPassword && 'border-pink_primary'
					}`}
				>
					<span
						className={`flex justify-center items-center w-6 h-6 sm:w-8 sm:h-8 text-white ${
							isPassword ? 'bg-green_primary' : 'bg-pink_primary'
						} rounded-full`}
					>
            1
					</span>
				...
				<div
					className={`font-medium text-grey_primary flex flex-col gap-y-2 text-xs justify-center items-center sm:flex-row sm:text-sm gap-x-2 border-b-2 pb-2 ${
						isPassword && 'border-pink_primary'
					}`}
				>
					<span
						className={`flex justify-center items-center w-6 h-6 sm:w-8 sm:h-8 text-white ${
							isPassword ? 'bg-pink_primary' : 'bg-grey_secondary'
						} rounded-full`}
					>
            2
					</span>
			...
			{defaultWallets.length === 0 && isDelegation && (
				<Alert
					...
					type="info"
					showIcon
					className="text-[#243A57] changeColor"
				/>
			)}
			{walletError && <Alert message={walletError} type="error" />}
			<AuthForm onSubmit={handleSubmitForm} className="flex flex-col gap-y-6">
				{isPassword ? (
					<>
            ...
            Already have an account?
					...
					<div
						onClick={() => handleClick()}
						className="text-pink_primary text-md cursor-pointer"
					>
            Login
					</div>
				</div>
			...
			<Modal
				...
				footer={[
					<div className="w-full flex justify-center" key="got-it">
						<Button
							icon={<CheckOutlined />}
							className="bg-pink_primary text-white outline-none border-none rounded-md px-5 font-medium text-lg leading-none flex items-center justify-center"
							onClick={() => {
								setOpen(false);
								!isModal && router.back();
							}}
			...
			>
        We sent you an email to verify your address. Click on the link in the
        email.
			</Modal>
		</article>
	);
