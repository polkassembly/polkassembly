...
import TooltipContent from '~src/components/Post/ActionsBar/Reactionbar/TooltipContent';
...
const BountyPostReactionButtons: FC<IReactionButtonProps> = ({
	className,
	reaction,
	commentId,
	reactions,
	...
	const handleReact = async () => {
		if (!id || !username) {
			...
			const { data, error } = await nextApiClientFetch<MessageType>(`api/v1/auth/actions/${actionName}`, {
				commentId: commentId,
				postId: postIndex,
				postType: ProposalType.USER_CREATED_BOUNTIES,
				reaction,
...
export default BountyPostReactionButtons;
