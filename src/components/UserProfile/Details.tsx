          ...
          Object.entries(identity).forEach(([key, value]) => {
            if (value) {
              if (
                Array.isArray(value) &&
                value.length > 0 &&
                (onChainIdentity as any)?.[key]?.length === 0
              ) {
    ...
    <div className="w-full md:w-auto h-full flex flex-col gap-y-5 bg-[#F5F5F5]">
      <article className="md:w-[330px] bg-[#910365] rounded-l-[4px] md:flex-1 py-[22px] md:py-8 px-4">
        <div className="flex flex-col items-center w-full">
          ...
          <div className="flex gap-2 text-xl items-center justify-center">
            <h2
              title={newUsername}
              className="font-semibold text-xl text-white truncate max-w-[200px] mt-[18px]"
            >
              {newUsername}
            </h2>
            {isGood && onChainIdentity.judgements.length > 0 && (
              <CheckCircleFilled
                style={{ color: 'green' }}
                className="rounded-[50%] bg-white h-[20px] border-solid border-[#910365] mt-[7px]"
              />
            )}
          </div>
          <div className="flex items-center text-xl text-navBlue gap-x-5 md:gap-x-3 mt-">
            {socialLinks?.map((social, index) => {
              const link =
                social_links && Array.isArray(social_links)
                  ? social_links?.find((s) => s.type === social)?.link || ''
                  : '';
              return (
                <SocialLink
      ...
      <div className="md:hidden flex-1 bg-white rounded-[4px] px-4">
        <Tabs
          ...
          items={[
            {
              children: <About title={title} bio={bio} addresses={addresses} />,
              key: 'about',
              label: 'About',
            },
            {
