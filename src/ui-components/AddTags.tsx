...
const AddTags=({ tags,setTags,className }:Props) => {
	...
	const handleInputConfirm = () => {
		...
		if(selectedTag.current !== null){
			...
			if ( inputValue && tags.length < 5 && tags.indexOf( inputValue ) === -1 && inputValue.trim().length > 0){
          ...
          Add new tag
					...
					<div className='max-sm:flex max-sm:flex-col max-sm:gap-1 max-sm:mt-1'>
						{tags.map((tag,index) => (
							<Tag
								key={index}
								className='text-[#90A0B7] rounded-xl bg-white text-normal text-xs py-[4px] px-[16px] tracking-wide hover:border-pink_primary'
								closable
