						...
						: <>
							<div className={'description display_inline flex items-center w-fit'}>
								...
								<span title={mainDisplay || encoded_addr} className={` max-w-[150px] flex gap-x-1 ${textClassName}`}>
									{ t1 && <span className={` ${truncateUsername && 'truncate'} text-[#243a57]  ${identity && mainDisplay && '-ml-1.5'}`}>{ t1 }</span> }
									{sub && isSubVisible && <span className={`sub ${isShortenAddressLength && 'truncate'} text-[#243a57]`}>{sub}</span>}
								</span>
							</div>
						</>
