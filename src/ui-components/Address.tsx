...
const Address = ({ address, className, displayInline, disableIdenticon, extensionName, popupContent, disableAddress, textClassName, shortenAddressLength, isShortenAddressLength = true, identiconSize, ethIdenticonSize, disableHeader, disableAddressClick, isSubVisible = true, addressClassName, clickable=true , truncateUsername = true }: Props): JSX.Element => {
	...
	const FEATURE_RELEASE_DATE = dayjs('2023-06-12').toDate(); // Date from which web3 address is being used as username on web3 signup.
	const manualUsernames25char: string[] = ['marcosdanieldasilvasantos','masterquincyleroyharrison','sasha.fleur.polkaassembly'];
	...
	const fetchUsername = async (isOnclick:boolean) => {
		...
		if (substrateAddress) {
			...
			if(isOnclick){
				...
				if(data && data.username && data.custom_username){
					setUsername(data.username);
				...
				if (data && data.username) {
					...
					if(manualUsernames25char.includes(data.username)){
						setIsAutoGeneratedUsername(false);
					}
					else if(data.username.length !== 25){
						setIsAutoGeneratedUsername(false);
						if(data.username.length > 25){
							setUsername(shortenAddress(data.username,8));
							return;
						}
					}
					else if(data.web3Signup && !data.created_at && data.username.length === 25  ){
						setIsAutoGeneratedUsername(true);
					}
					else if(data.web3Signup && data.username.length === 25 &&  dayjs(data.created_at).isBefore( dayjs(FEATURE_RELEASE_DATE)) ){
						setIsAutoGeneratedUsername(true);
					}
				}
			}
	...
	useEffect(() => {
		...
		api.derive.accounts.info(encoded_addr, (info: DeriveAccountInfo) => {
			...
			if (info.identity.displayParent && info.identity.display){
				...
				setMainDisplay(info.identity.displayParent || info.identity.display|| (!isAutoGeneratedUsername ? username : null) || info.nickname || '' );
			}
		})
	...
	const t1 = kiltName || mainDisplay || (!isAutoGeneratedUsername ? username : null) || (isShortenAddressLength? shortenAddress(encoded_addr, shortenAddressLength): encoded_addr) || username;
						...
						: <>
							<div className={'description display_inline flex items-center w-fit'}>
								...
								<span title={mainDisplay || encoded_addr} className={` identityName max-w-[150px] flex gap-x-1 ${textClassName}`}>
									{ t1 && <span className={` ${truncateUsername && 'truncate'} text-navBlue  ${identity && mainDisplay && '-ml-1.5'}`}>{ t1 }</span> }
									{sub && isSubVisible && <span className={`sub ${isShortenAddressLength && 'truncate'} text-navBlue`}>{sub}</span>}
								</span>
							</div>
						</>
