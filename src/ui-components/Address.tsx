...
const Address = ({ address, className, displayInline, disableIdenticon, extensionName, popupContent, disableAddress, textClassName, shortenAddressLength, isShortenAddressLength = true, identiconSize, ethIdenticonSize, disableHeader, disableAddressClick, isSubVisible = true, addressClassName, clickable=true , truncateUsername = true }: Props): JSX.Element => {
	...
	const featureReleaseDate = dayjs('2023-06-08').toDate(); // Date from which web3 address is being used as username on web3 signup.
			...
			if (data && data.username) {
				...
				if(ManualUsernames25char.includes(data.username)){
					...
					if(data.username.length > 25){
						setManualUserName(shortenAddress(data.username,8));
						return;
					}
					setManualUserName(data.username);
				}
				else if(data.web3Signup && !data.created_at && data.username.length === 25  ){
					setAutoGeneratedUsername(data.username);
					setManualUserName('');
				}
				else if(data.web3Signup && data.username.length === 25 &&  dayjs(data.created_at).date() < dayjs(featureReleaseDate).date() && dayjs(data.created_at).month() < dayjs(featureReleaseDate).month() && dayjs(data.created_at).year() < dayjs(featureReleaseDate).year() ){
					setAutoGeneratedUsername(data.username);
					setManualUserName('');
				}
			}
	...
	const t1 = kiltName || mainDisplay || manualUserName || (isShortenAddressLength? shortenAddress(encoded_addr, shortenAddressLength): encoded_addr) || autoGeneratedUsername;
						...
						: <>
							<div className={'description display_inline flex items-center w-fit'}>
								...
								<span title={mainDisplay || encoded_addr} className={` identityName max-w-[150px] flex gap-x-1 ${textClassName}`}>
									{ t1 && <span className={` ${truncateUsername && 'truncate'} text-[#9eacc0]  ${identity && mainDisplay && '-ml-1.5'}`}>{ t1 }</span> }
