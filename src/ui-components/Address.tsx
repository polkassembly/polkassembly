...
const Address = ({
    ...
    const [isAutoGeneratedUsername, setIsAutoGeneratedUsername] =
        useState(true);
        ...
        if (substrateAddress) {
            try {
                ...
                if (
                    ...
                    (data.web3Signup &&
                        data.username.length === 25 &&
                        dayjs(data.created_at).isBefore(
                            dayjs(FEATURE_RELEASE_DATE)
                        ))
                ) {
    ...
    const t1 =
        kiltName ||
        mainDisplay ||
        (!isAutoGeneratedUsername ? shortenUsername(username) : null) ||
        (isShortenAddressLength
            ? shortenAddress(encoded_addr, shortenAddressLength)
            : encoded_addr) ||
        shortenUsername(username);
    ...
    return (
        <div
            className={
                displayInline ? `${className} display_inline` : className
            }
        >
            {!disableIdenticon ? (
            ...
            {!disableAddress && (
                ...
                >
                    {displayInline ? (
                        ...
                        popupContent ? (
                            <Space>
                                ...
                                <Tooltip color="#E5007A" title={popupContent}>
                                    <div
                                        className={
                                            "header display_inline identityName max-w-[30px] flex flex-col gap-y-1  text-navBlue"
                                        }
                                    >
                                        {t1 && (
                                            <span
                                        ...
                                        {sub && isSubVisible && (
                                            <span
                                                className={`sub truncate ${textClassName}`}
                                            >
                                                {sub}
                                            </span>
                                        )}
                            ...
                            <>
                                <div
                                    className={
                                        "description display_inline flex items-center"
                                    }
                                >
                                    {identity && mainDisplay && (
                                        <IdentityBadge
                                            ...
                                            identity={identity}
                                            flags={flags}
                                            className="mr-2 text-navBlue"
                                        />
                                    )}
                                    <span
                                        title={mainDisplay || encoded_addr}
                                        className={` identityName max-w-[85px] flex gap-x-1 ${textClassName}`}
                                    >
                                        {t1 && (
                                            <span
                                                className={`${
                        ...
                        popupContent ? (
                            <Tooltip color="#E5007A" title={popupContent}>
                                <Space>
                                    <Space className={"header"}>
                                        ...
                                        <span className="bg-red-500 identityName max-w-[85px] flex flex-col gap-y-1 text-navBlue">
                                            ...
                                            {!extensionName &&
                                                ...
                                                isSubVisible && (
                                                    <span
                                                        className={`${textClassName} sub truncate text-navBlue`}
                                                    >
                                                        {sub}
                                                    </span>
                                                )}
                                    ...
                                    <div
                                        className={"description display_inline"}
                                    >
                                        {isShortenAddressLength
                                            ? shortenAddress(
                                                  encoded_addr,
                            ...
                            <div>
                                {!disableHeader ? (
                                    <Space className={"header"}>
                                        ...
                                        <span className="identityName max-w-[85px] flex flex-col gap-y-1">
                                            {t2 && (
                                                <span
                                                    className={`${textClassName} truncate text-[#243a57] font-semibold`}
                                                >
                                                    {t2}
                                                </span>
                                            )}
                                            {!extensionName &&
                                                ...
                                                isSubVisible && (
                                                    <span
                                                        className={`${textClassName} sub truncate text-[#243a57] font-semibold`}
                                                    >
                                                        {sub}
                                                    </span>
                                                )}
                                ...
                                <div
                                    className={`description text-xs ml-0.5 ${addressClassName}`}
                                >
                                    {isShortenAddressLength
                                        ? shortenAddress(
                                              encoded_addr,
                        ...
                        <div
                            className={`description text-xs ${addressClassName}`}
                        >
                            {kiltName
                                ? t1
                                : isShortenAddressLength
            ...
            {otherTextType ? (
                <p
                    className={`m-0 flex items-center gap-x-1 text-lightBlue leading-[15px] text-[10px] ${otherTextClassName}`}
                >
                    <span
                        className={classNames("w-[6px] h-[6px] rounded-full", {
                            "bg-aye_green ": [
                                EAddressOtherTextType.CONNECTED,
                                EAddressOtherTextType.COUNCIL_CONNECTED
                            ].includes(otherTextType),
                            "bg-blue ":
                                otherTextType === EAddressOtherTextType.COUNCIL,
                            "bg-nay_red":
                                otherTextType ===
                                EAddressOtherTextType.LINKED_ADDRESS
                        })}
                    ></span>
                    <span className="text-xs">{otherTextType}</span>
                </p>
            ) : null}
        </div>
    );
