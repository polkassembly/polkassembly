...
const Address = (props: Props) => {
	...
	const handleIdentityInfo = useCallback(() => {
		...
		(async () => {
			...
			setIdentity((prev) => (JSON.stringify(prev) !== JSON.stringify(info) ? info : prev));
			if (info.display) {
	...
	}, [api, peopleChainApi, address, network, username, usernameMaxLength, isAutoGeneratedUsername]);
	...
	const addressPrefix = useMemo(
		...
		[kiltName, mainDisplay, isAutoGeneratedUsername, username, showFullAddress, encodedAddr, addressMaxLength, usernameMaxLength]
	);

	const addressSuffix = useMemo(() => extensionName || mainDisplay, [extensionName, mainDisplay]);
