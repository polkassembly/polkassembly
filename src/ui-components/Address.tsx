...
const Address = ({
	...
	const fetchUsername = async (isOnclick: boolean) => {
		...
		if (substrateAddress) {
          ...
          (data.web3Signup &&
            data.username.length === 25 &&
            dayjs(data.created_at).isBefore(dayjs(FEATURE_RELEASE_DATE)))
				) {
					...
					// When inline disregard the extension name.
						popupContent ? (
							<Space>
								...
								<Tooltip color="#E5007A" title={popupContent}>
									...
									>
										{t1 && (
											<span className={`truncate ${textClassName}`}>{t1}</span>
										)}
										{sub && isSubVisible && (
											<span className={`sub truncate ${textClassName}`}>
												{sub}
											</span>
										)}
									</div>
							...
							<>
								<div className={'description display_inline flex items-center'}>
									...
									>
										{t1 && (
											<span
												className={`${truncateUsername && 'truncate'} ${
													identity && mainDisplay && '-ml-1.5'
												}`}
											>
												{t1}
											</span>
                      ...
                      !extensionName && (
										...
										<span className="bg-red-500 identityName max-w-[85px] flex flex-col gap-y-1 text-navBlue">
											{t2 && (
												<span className={`${textClassName} truncate `}>
													{t2}
												</span>
											)}
									...
									<div className={'description display_inline'}>
										{isShortenAddressLength
											? shortenAddress(encoded_addr, shortenAddressLength)
											: encoded_addr}
									</div>
                      ...
                      !extensionName && (
						...
						<div className={`description text-xs ${addressClassName}`}>
							{kiltName
								? t1
								: isShortenAddressLength
									? shortenAddress(encoded_addr, shortenAddressLength)
				...
				>
					<span
						className={classNames('w-[6px] h-[6px] rounded-full', {
							...
							'bg-nay_red':
                otherTextType === EAddressOtherTextType.LINKED_ADDRESS
						})}
					></span>
