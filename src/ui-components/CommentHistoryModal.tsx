...
const CommentHistoryModal = ({
    ...
    const items: TimelineItemProps[] = historyData?.map((item, index) => {
        const difference = historyData[index + 1]
            ? diffChars(historyData[index + 1]?.content, item?.content)
            : [];
        ...
        const items: MenuProps["items"] = [
            item?.sentiment === 1
                ? {
                      ...
                      label: (
                          <div
                              className={`${poppins.variable} ${poppins.className} text-[10px] leading-4 bg-pink-100 font-light pl-1 pr-1 tracking-wide`}
                          >
                              Completely Against
                          </div>
                      )
        ...
        return {
            children: (
                <div
                    className={`py-3 pl-3 pr-1 bg-[#FAFAFC] rounded-[4px] ml-2 max-sm:w-full max-sm:ml-0 ${
                ...
                >
                    <div className="flex justify-between items-center">
                        <div className="flex items-center">
                            ...
                            <div className="flex items-center">
                                ...
                                <div className="flex items-center">
                                    <span className="text-navBlue text-[10px]">
                        ...
                        {item?.sentiment === 1 && (
                            <Dropdown
                                ...
                                placement="topCenter"
                                menu={{ items }}
                                className="text-lg text-white  flex justify-center items-center  min-[320px]:mr-2"
                            >
                        ...
                        {item?.sentiment === 5 && (
                            <Dropdown
                                ...
                                placement="topCenter"
                                menu={{ items }}
                                className="text-[20px] mr-[-1px] mb-[-1px] mt-[-2px] text-white  flex justify-center items-center min-[320px]:mr-2"
                            >
                    ...
                    <div
                        className={`mt-2 text-[#243A57] text-sm px-[2px] font-normal ${
                            !item?.expanded &&
                            item?.content.length > 100 &&
                            "truncate-content"
                        } tracking-[0.01em] ${poppins.className} ${
                            poppins.variable
                        } leading-6 pr-2`}
                    >
                        {historyData[index + 1] ? (
                            <div>
                                {difference?.map((text, idx) => (
                                    <span
                                        key={idx}
                                        className={`${
                                            text?.removed && "bg-[#fff3b3]"
                                        } ${text?.added && "bg-[#fff3b3]"}`}
                                    >
                                        {text.value}
                                    </span>
                                ))}
                    ...
                    {item?.content.length > 100 && (
                        <span
                            ...
                            className="text-xs cursor-pointer text-[#E5007A] font-medium mt-1"
                        >
    ...
    return (
        <Modal
            ...
            closeIcon={<CloseIcon />}
            title={
                <label className="-mt-2 pr-3 text-[#334D6E] text-[20px] font-semibold ">
...
export default styled(CommentHistoryModal)`
    ...
    .post-history-timeline
        .ant-timeline
        .ant-timeline-item:has(.active-timeline) {
