...
const CreationLabel: FC<ICreationLabelProps> = (props) => {
	...
	const AbstainDetailsComponent = ({ network, vote, power }: any) => {
		return (
			<>
				<div className={'abstain-amount-value dark:text-blue-dark-high ml-[64px] w-[92px] overflow-ellipsis text-center text-bodyBlue'}>
					{parseBalance((vote?.balance?.abstain || 0).toString(), 2, true, network)}
				</div>
				<div className={'abstain-conviction-value dark:text-blue-dark-high ml-[44px] mr-[50px] w-[92px] overflow-ellipsis text-center text-bodyBlue'}>-</div>
				<div className='abstain-power-value dark:text-blue-dark-high w-[92px] overflow-ellipsis text-center text-bodyBlue'>{power}</div>
			</>
		);
	};
	...
	const AyeNyeDetailsComponent = ({ network, vote, power }: any) => {
		return (
			<>
				<div className={'amount-value dark:text-blue-dark-high ml-[95px] w-[92px] overflow-ellipsis text-center text-bodyBlue'}>
					{parseBalance((vote?.balance?.value || 0).toString(), 2, true, network)}
				</div>
				<div className={'conviction-value dark:text-blue-dark-high ml-10 mr-[55px] w-[92px] overflow-ellipsis text-center text-bodyBlue'}>{`${
					vote.lockPeriod === 0 ? '0.1' : vote.lockPeriod
				}x`}</div>
				<div className='power-value dark:text-blue-dark-high -mr-[60px] w-[92px] overflow-ellipsis text-center text-bodyBlue'>{power}</div>
			</>
		);
	};
	const SplitDetailsComponent = ({ network, vote, power }: any) => {
		return (
			<>
				<div className={'amount-value dark:text-blue-dark-high ml-[86px] w-[92px] overflow-ellipsis text-center text-bodyBlue'}>
				...
				{vote?.decision === 'abstain' && (
					<div className={'conviction-value dark:text-blue-dark-high ml-10 mr-[58px] w-[92px] overflow-ellipsis text-center text-bodyBlue'}> - </div>
				)}
				<div className='power-value dark:text-blue-dark-high -mr-[60px] w-[92px] overflow-ellipsis text-center text-bodyBlue'>{power}</div>
			</>
		);
				...
				{vote.decision == 'yes' ? (
					...
					<div className=' align-center mb-[1px] flex w-[90%] justify-between'>
						<div className='flex justify-center align-middle'>
							<AbstainGray className='mr-1' /> <span className='dark:text-blue-dark-high font-medium capitalize text-bodyBlue'>Abstain</span>
	...
	return (
		<div className={`${className} flex w-[100%] justify-between bg-none`}>
			<div className={`flex text-xs ${isRow ? 'flex-row' : 'flex-col'} max-sm:flex-wrap max-sm:gap-1 md:flex-row md:items-center`}>
					...
					<div className={'flex flex-shrink-0 items-center'}>
						...
						{topic && (
							<div className='flex items-center sm:-mt-0.5'>
								<span className='dark:text-blue-dark-medium mr-2 mt-0.5 text-lightBlue'>in</span>{' '}
								<TopicTag
									topic={topic}
									className={topic}
									theme={theme}
				...
				<div className='dark:text-blue-dark-medium flex items-center text-lightBlue max-xs:ml-1'>
					...
					{vote && (
						<div className='flex items-center justify-center'>
							...
							{vote === EVoteDecisionType.AYE ? (
								...
								<div className='align-center mb-[1px] flex justify-center'>
									<AbstainGray className='mr-1' /> <span className='ont-medium dark:text-blue-dark-high capitalize text-bodyBlue'>Voted {vote}</span>
								</div>
							) : null}
						</div>
					)}
						...
						>
							...
							{votesArr[0].decision == 'yes' ? (
								...
								<div className='abstain-voted-icon voted-icon align-center mb-[1px] flex justify-center'>
									<AbstainGray className='mb-[-1px] mr-1' /> <span className='dark:text-blue-dark-high mt-[2px] font-medium capitalize text-bodyBlue'>Voted Abstain</span>
								</div>
							) : null}
							...
							<Modal
								...
								footer={false}
								className={`w-[400px] ${poppins.variable} ${poppins.className} dark:[&>.ant-modal-content]:bg-section-dark-overlay max-h-[675px] rounded-[6px] max-md:w-full`}
								...
								title={
									<div className='dark:border-separatorDark dark:bg-section-dark-overlay -mt-5 ml-[-24px] mr-[-24px] flex h-[65px] items-center gap-2 rounded-t-[6px] border-0 border-b-[1.5px] border-solid border-[#D2D8E0]'>
										<span className='dark:text-blue-dark-high ml-4 text-xl font-semibold tracking-[0.0015em] text-bodyBlue'>Votes</span>
									</div>
								}
							>
								<div className='modal-content'>
									<div className='modal-container dark:text-blue-dark-high mt-3 flex text-sm font-semibold text-bodyBlue'>
									...
									<div>
										{votesArr.length > 1 && (
											<div className='vote-history-container'>
												...
												<p className='dark:text-blue-dark-high m-0 mb-2 p-0 text-sm font-semibold text-bodyBlue'>Vote History</p>
											</div>
										)}
										{votesArr.length > 1 &&
