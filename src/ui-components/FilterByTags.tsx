...
const FilterByTags=({ className, isSearch, setSelectedTags, disabled }:Props) => {
	...
	return (
        ...
        Filter
			...
			</div> : <div className={`flex items-center justify-center text-xs ${(openFilter || tags?.length > 0) ? 'text-pink_primary':'text-[#667589]'} ${disabled ? 'text-[#B5BFCC] cursor-not-allowed' : 'cursor-pointer'}`}>
          Tags
