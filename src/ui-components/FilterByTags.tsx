...
const FilterByTags = ({
	...
	const content = (
		<div className="min-h-[150px] w-[180px] ">
			{!isSearch ? (
				<div
					className={`text-sidebarBlue cursor-auto flex text-sm justify-between font-medium mb-[-2px] mt-[-2px] tracking-wide ${poppins.variable} ${poppins.className}`}
				>
          Tags
					{!isSearch && (
						<span
							className="text-pink_primary font-normal text-[10px] flex justify-center cursor-pointer"
							onClick={() => {
								setTags([]);
      ...
      filteredTags.length === 0 ? (
					<div className="flex-col">
						{isSearch && (
							<div
								className={`text-[10px] text-[#243A57] font-normal mt-1 ${poppins.variable} ${poppins.className}`}
							>
              Suggestion :
							</div>
						)}
						...
						{trendingTags.slice(0, 5).map((tag, index) => (
							<div
								...
								className={`flex gap-2 text-xs items-center py-1 cursor-pointer ${poppins.className} ${poppins.variable}`}
							>
								<TrendingIcon />
					...
					<Checkbox.Group
						className={`flex flex-col mt-1.5 tracking-[0.01em] justify-start max-h-[200px] overflow-y-scroll  ${poppins.className} ${poppins.variable}`}
						value={tags}
					>
						{displayTags.map((item, index) => (
							<Checkbox
								onClick={() =>
									handleExits(item) ? handleRemoveTag(item) : handleSetTags(item)
								}
								className={`text-xs font-normal ml-0 ${
									tags.includes(item) ? 'text-[#243A57]' : 'text-[#667589]'
			...
			{filteredTags.length === 0 && searchInput.length > 0 ? (
				<div className="h-[100%] flex items-center justify-center flex-col gap-2 mt-2">
					<NoTagsFoundIcon />
					<span
						className={`text-[10px] text-navBlue tracking-wide ${poppins.className} ${poppins.variable} `}
					>
            No tag found.
					</span>
				</div>
	...
	return (
		...
		>
			{!isSearch ? (
				<div
					className={
						'text-sm tracking-wide font-normal flex items-center text-pink_primary mt-[3.5px] cursor-pointer'
					}
				>
					<span> Filter</span>
				...
				<div
					className={`flex items-center justify-center text-xs ${
						openFilter ? 'text-pink_primary' : 'text-[#667589]'
					} ${
						disabled ? 'text-[#B5BFCC] cursor-not-allowed' : 'cursor-pointer'
					} max-sm:text-[10px]`}
				>
          Tags
					<span className="text-[#96A4B6] font-semibold">
						{openFilter ? (
							<HightlightDownOutlined className="ml-2.5 mt-1 max-md:ml-1" />
						) : (
