...
const FilterByTags=({ className }:Props) => {
	...
	const items:MenuProps['items'] = [
		{ key: 1,label:
		<div className={`text-bodyBlue cursor-auto flex text-sm justify-between font-medium mb-[-4px] mt-[-2px] tracking-wide ${poppins.variable} ${poppins.className}`}>
			Tags
			<span className='text-pink_primary font-normal text-[10px] flex justify-center cursor-pointer'
				onClick={(e) => {e.preventDefault(); e.stopPropagation();setTags([]);handleFilterByClick([]);setSearchInput('');}}>Clear Filters</span></div> },
		{ key: 2, label:
		<Input onClick={(e) => {e.preventDefault();e.stopPropagation();}} type='search' value={searchInput} onChange={(e) => setSearchInput(e.target.value)} prefix={<SearchIcon/>} /> },
		...tags.map((tag,index) =>
		{
			return {
				key: index+3, label:
				<div className='flex items-center justify-between boder-solid'>
					<div>
						<SearchIcon className='mr-2'/>
						<span className={`${poppins.variable} ${poppins.className} text-navBlue text-xs tracking-wide`}>{tag}</span>
					</div>
					<div>{handleExits(tag)?
						<div onClick={(e) => {e.preventDefault(); e.stopPropagation();handleRemoveTag(tag);}}><CheckedIcon className='mt-[-2px]'/></div>:<div onClick={(e) => {e.preventDefault(); e.stopPropagation(); handleSetTags(tag);}}><CheckOutlineIcon className='mt-[-2px]'/></div>}</div></div>
			};
		}),
		...trendingTags.slice(0,5).map((tag, index) => {
			if(searchInput.length === 0 && tags.length === 0 && filteredTags.length === 0)
			{
				return { key:index+10,label:<div onClick={(e) => {e.preventDefault(); e.stopPropagation();handleSetTags(tag?.name);}} className={`flex gap-2 text-xs items-center ${poppins.className} ${poppins.variable}`}><TrendingIcon/><span className='text-xs text-navBlue tracking-wide'>{tag.name}</span></div> };
			}
			return null;
		}),
		...
		...filteredTags.slice(0,5).map((tag, index) => {
			return { key: index+20, label:<div className='flex items-center justify-between'><div><SearchIcon className='mr-2'/><span className={`${poppins.variable} ${poppins.className} text-navBlue text-xs tracking-wide`}>{tag?.name}</span></div><div>{!handleExits(tag?.name) && <div onClick={(e) => {e.preventDefault(); e.stopPropagation();handleSetTags(tag?.name);}}><CheckOutlineIcon className='mt-[-2px]'/></div>}</div></div> };
		})
	];
