...
const Gif: FC<IGifProps> = (props) => {
	...
	return (
		<Spin className='min-h-[480px]' spinning={loading} indicator={<LoadingOutlined />}>
			<section className={classNames('flex flex-col gap-y-5 min-h-[480px]', props.className)}>
				{
					gifResult ?
						<>
							<article>
							...
							<article className='grid grid-cols-3 gap-5'>
								{
									gifResult.data && gifResult.data.map((gif: any) => {
										return (
											<button
												onClick={() => {
													if (props.onClick) {
												...
												key={gif.id}
												className='cursor-pointer border-none outline-none bg-transparent flex items-center justify-center h-28'
											>
												<img className='w-full h-full object-cover' src={gif.images.original.url} alt={gif.title} />
											</button>
										);
									})
								}
