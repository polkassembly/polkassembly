...
const IdentityBadge = ({
	...
	const color: 'brown' | 'green' | 'grey' = isGood
		...
		: 'grey';
	const CouncilEmoji = () => (
		<span aria-label="council member" className="-mt-1" role="img">
			👑
		</span>
	);
	const infoElem = (
		...
		<StyledPopup>
			{identity?.legal && (
				<li className="flex items-center">
					...
					<span className="text-xs text-bodyBlue font-normal pt-0.5">
						{identity.legal}
					</span>
				</li>
			)}
			...
			{(identity?.judgements?.length || 0) > 0 && (
				<li className="flex items-center">
					<span className="desc flex items-center text-sm text-bodyBlue font-medium">
			...
			{identity?.pgp && (
				<li className="flex items-center">
					...
					<span className="text-xs text-bodyblue font-normal">
						{identity.pgp}
					</span>
				</li>
			)}
			...
			{identity?.twitter && (
				<li className="flex items-center">
					<span className="desc text-bodyBlue font-medium flex text-sm">
						<TwitterIcon className="mr-1.5 mt-1" />
						Twitter:{' '}
					</span>
					<span className="text-xs font-normal text-bodyBlue">
						{identity.twitter}
					</span>
				</li>
			)}
			{identity?.web && (
				<li className="flex items-center">
					<span className="desc text-bodyBlue font-medium flex text-sm">
						<WebIcon className="mr-1.5 mt-1 -ml-0.5" />
						Web:{' '}
					</span>
					<span className="text-xs text-bodyBlue font-normal pt-0.5">
						{identity.web}
			...
			{flags?.isCouncil && (
				<li className="flex items-center">
					<span className="desc text-bodyBlue font-medium text-sm">
				...
				<li className="flex items-center">
					<span className="desc">
						<a
							...
							target="_blank"
							rel="noreferrer"
							className="flex text-pink-500 underline items-center"
						>
