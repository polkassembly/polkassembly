...
for (const key of Object.keys(network)) {
	...
	link = ['MOONBASE', 'MOONRIVER', 'MOONBEAM', 'KILT'].includes(key)
		...
		: `https://${
			key === 'POLYMESHTEST' ? 'polymesh-test' : keyVal
		}.polkassembly.io`;
			...
			dropdownRender={() => {
				return (
					<Card className="max-w-[356px] max-h-[52vh] overflow-y-auto">
						<>
							<div className="text-bodyBlue font-medium">
                Polkadot &amp; Parachains
							</div>
							<Row className="mt-2">
								{polkadotChains.map((optionObj) => (
									<Col
										span={12}
										key={optionObj.key}
										className={`flex ${!isSearch && 'cursor-pointer'} ${
											isSearch &&
                      selectedNetworks?.includes(optionObj.key) &&
                      'text-pink_primary font-medium cursor-pointer'
										} ${
											isSearch &&
                      !allowedNetwork?.includes(optionObj?.key) &&
                      'text-[#B5BFCC] cursor-not-allowed'
							...
							<div className="text-bodyBlue font-medium mt-4">
                Kusama &amp; Parachains
							</div>
							<Row className="mt-2">
          ...
          Network
					<span className="text-[#96A4B6]">
						{openFilter ? (
							...
							<DownOutlined className="ml-2.5 max-md:ml-1 mt-1" />
						)}
					</span>
				</div>
			) : isSmallScreen ? (
				<a
					className="flex items-center justify-between gap-x-2 rounded-[4px] border border-solid border-[#D2D8E0] bg-[rgba(210,216,224,0.2)] h-10 px-[18px]"
					onClick={(e) => {
						e.preventDefault();
						setSidedrawer(false);
					}}
				...
				<a
					className="flex items-center justify-between text-bodyBlue hover:text-pink_primary lg:min-w-[133px] lg:h-8 lg:border-solid lg:border lg:border-[#D2D8E0] lg:rounded-[26px] lg:bg-[rgba(210,216,224,0.2)] lg:px-[12px] lg:py-[6px]"
					onClick={(e) => {
						e.preventDefault();
						setSidedrawer(false);
					}}
