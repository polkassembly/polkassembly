...
const SetNotificationNudge = ({ className }: Props) => {
	...
	useEffect(() => {
		...
		const nudgeStatus = localStorage.getItem('notificationNudgeStatus');
	...
	return (
		<div className={`${className} flex flex-row border-none bg-[#2D80FF]`}>
			...
			<div className='ant-layout-content mx-auto flex w-[94vw] max-w-7xl flex-initial flex-row items-center justify-between gap-8 px-2 py-2 lg:w-[85vw] 2xl:w-5/6'>
				...
				>
					...
					<span className='inline-flex cursor-pointer items-center gap-2 rounded-md bg-[#000000]/30 px-2 py-1 hover:opacity-80'>Set Notifications</span>
				</div>
				<span
					className='ml-auto'
...
export default SetNotificationNudge;
