...
const formatClassName = (track: string): string => {
	return track.replace(/\s+/g, '');
};

const TrackTag = ({ className, track }: Props) => {
	const formattedTrack = formatClassName(track);

	return <Tag className={`${className} ${formattedTrack} px-3 py-1 text-xs`}>{track}</Tag>;
};

export default styled(TrackTag)`
	font-weight: 500;
