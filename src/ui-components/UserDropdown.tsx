		...
		<div className={`${displayInline ? className + ' display_inline': className} bg-[#f6f7f9] rounded-3xl user-container px-2 flex justify-center items-center`} style={{ border:'1px solid #d7dce3' }}>
			{
				!disableIdenticon ?
					encoded_addr.startsWith('0x') ?
						...
						:
						<Identicon
							className='image identicon user-image-container'
							value={encoded_addr}
			...
			{!disableAddress && <div className={`content ${clickable ? 'cursor-pointer' : 'cursor-not-allowed' }`} onClick={async () => {
				...
				// When inline disregard the extension name.
					...
					: extensionName || mainDisplay
						...
						: <div className={`description ${addressClassName} w-[60px] user-details-container text-ellipsis overflow-hidden text-xs`}>{kiltName ? t1 : isShortenAddressLength? shortenAddress(encoded_addr, shortenAddressLength): encoded_addr}</div>
				}
			</div>}
			{
