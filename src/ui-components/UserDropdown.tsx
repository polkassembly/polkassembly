...
const UserDropdown = ({ isSimpleDropdown, address, className, displayInline, disableIdenticon, extensionName, popupContent, disableAddress, textClassName, shortenAddressLength, isShortenAddressLength = true, identiconSize, ethIdenticonSize, disableHeader, disableAddressClick, isSubVisible = true, addressClassName, clickable=true , truncateUsername = true, otherTextType, otherTextClassName, passedUsername = '', isVoterAddress }: Props): JSX.Element => {
	...
	return (
		<div className={`${displayInline ? className + ' display_inline': className} ${!isSimpleDropdown && 'bg-[#f6f7f9] rounded-3xl'}  px-2 flex justify-start items-center`} style={{ border:isSimpleDropdown?'unsest':'1px solid #d7dce3' }}>
			{
				!disableIdenticon ?
									...
									!disableHeader ?
										<Space className={'header'}>
											...
											<span className='identityName max-w-[85px] flex flex-col gap-y-1'>
												{ t2 && <span className={`${textClassName} truncate text-bodyBlue font-semibold font-poppins `}>{ t2 }</span> }
												{!extensionName && sub && isSubVisible && <span className={`${textClassName} sub truncate text-bodyBlue font-semibold font-poppins`}>{sub}</span>}
											</span>
										</Space>
										: null
								}
								<div className={`description ml-0.5 ${addressClassName} text-xs font-semibold font-poppins`}>{isShortenAddressLength? shortenAddress(encoded_addr, shortenAddressLength): encoded_addr}</div>
							</div>
						: <div className={`description ${addressClassName} text-xs font-semibold`}>{kiltName ? t1 : isShortenAddressLength? shortenAddress(encoded_addr, shortenAddressLength): encoded_addr}</div>
				}
			</div>}
			{
			...
			<div className='flex justify-center items-center'><svg xmlns="http://www.w3.org/2000/svg" width="30" height="25" viewBox="0 0 20 21" fill="none">
