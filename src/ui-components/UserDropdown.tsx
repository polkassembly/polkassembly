...
const UserDropdown = ({ address, className, displayInline, disableIdenticon, extensionName, popupContent, disableAddress, textClassName, shortenAddressLength, isShortenAddressLength = true, identiconSize, ethIdenticonSize, disableHeader, disableAddressClick, isSubVisible = true, addressClassName, clickable=true , truncateUsername = true, otherTextType, otherTextClassName, passedUsername = '', isVoterAddress }: Props): JSX.Element => {
	...
	return (
		<div className={`${displayInline ? className + ' display_inline': className} bg-[#f6f7f9] rounded-3xl px-2 flex justify-center items-center`} style={{ border:'1px solid #d7dce3' }}>
			{
				!disableIdenticon ?
			...
			{!disableAddress && <div className={`content ${clickable ? 'cursor-pointer' : 'cursor-not-allowed' }`} onClick={async () => {
				...
				// When inline disregard the extension name.
					...
					: extensionName || mainDisplay
						...
						: <div className={`description ${addressClassName} w-[60px] text-ellipsis overflow-hidden text-xs`}>{kiltName ? t1 : isShortenAddressLength? shortenAddress(encoded_addr, shortenAddressLength): encoded_addr}</div>
				}
			</div>}
			{
