...
export function getTopicFromType(proposalType: ProposalType) {
	...
	if ([ProposalType.DEMOCRACY_PROPOSALS, ProposalType.REFERENDUMS, ProposalType.OPEN_GOV].includes(proposalType)) {
		topic.id = post_topic.DEMOCRACY;
		topic.name = 'Democracy';
	} else if ([ProposalType.TREASURY_PROPOSALS, ProposalType.TIPS, ProposalType.BOUNTIES, ProposalType.CHILD_BOUNTIES].includes(proposalType)) {
...
export const isTopicIdValid = (topicId: any) => {
	...
	return !isNaN(numTopicId) && Object.keys(topicIdToNameMap).includes(String(topicId));
};

export function getTopicNameFromTopicId(topicId: keyof typeof topicIdToNameMap): string {
